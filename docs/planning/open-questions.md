...
When the user disables validation, the workbench must not run a hidden reduced readiness assessment, generate a readiness outcome or include `READINESS_REPORT.md`. The package-boundary preview states that readiness validation was not run. Included Risks, Assumptions, Open Questions, Direct External Dependencies and other selected Product Knowledge remain in their ordinary package representations; this is not a substitute caveat summary or readiness judgment. Scope eligibility and meaningful-product-definition gates remain non-optional.
