...
Each User Flow extension entry is headed by its readable reference and Title, then gives the primary User Profile name, Entry context and Intended outcome. Its **Journey steps** retain every Action, Surface, optional Relevant state and Feedback and continuation in the authored order, followed by non-empty **Intentional exits** and **Error recovery paths**. A Surface whose Screen / View is included in the package links to that extension entry. Otherwise, the step retains only plain `SV-### — Title` text marked **Outside package**; it neither links to nor adds the missing Screen / View detail. The projection excludes Feature and Functional Requirement relationships, Acceptance Criteria, User Profile detail, Screen / View detail, implementation routes and component structure. UI Requirement entry-level presentation remains a later decision.
