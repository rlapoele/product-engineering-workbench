...
During Prepare Handoff, the author chooses whether to include a Design System Companion and selects or accepts a visible default, versioned Export Profile. The first profile, Google `DESIGN.md` `alpha`, maps Definition metadata, intentional omissions, typed token collections and ordered prose to its target format. It preserves compatible references; renders configuration descriptions in Components guidance; keeps labels and tags as authoring metadata; and flattens each Component/configuration pair as `component--configuration`, blocking a duplicate output key rather than altering a source identifier. If Colors are included and no source color token is named `primary`, the author explicitly selects a source token as the profile's primary-color binding; the export emits a derived `primary` alias without changing canonical data. Target-incompatible values or references, an absent required binding and duplicate flattened component entries block that profile. Unknown component property names are retained with a warning. The selected inclusion decision, profile/version, bindings and generated companion are immutable prepared-handoff snapshot data, so the same Definition may be handed off later without a companion or with another supported profile. Richer component-property guidance and presentation refinements remain to be decided.
