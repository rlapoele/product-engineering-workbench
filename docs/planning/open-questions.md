...
Request detail is one structure ordered by Request Cycle, newest first. The current cycle is expanded, showing immutable recorded context and either its submitted response with labelled handling status or a compact current state. Private response drafts remain private. Earlier cycles are collapsed summaries by default. Expanding one immediately shows its summary and then loads its stored immutable Brief, recorded context and response inline beneath it. The detail shows a compact loading state and retry on retrieval failure, and caches expanded history only for the current detail session. Historical content must never be reconstructed from current Project State. The lifecycle record is limited to submission, start, response, decline, cancellation, On Hold, resume, reopen and response-handling changes; it is not an activity-feed or chat surface.

The requestor may Reopen a Responded or Declined request. Reopen is an action, not a status: it returns the request to a private Draft. The selected human recipient retains a frozen read-only view of prior shared request and response evidence, while the revised Brief and draft-context changes stay private until resubmission. On a Responded request, recipient, Assistance Request Type, scope and recorded context are read-only; only the Request Brief may change. On a Declined request, the owner may also choose a different eligible human recipient before resubmission. The replacement sees only the new current cycle, while the original recipient retains only their declined cycle. The owner may discard a reopened Draft to restore the preceding terminal cycle without erasing history. A recipient who Declines after starting response work discards their private draft. A Cancelled request cannot be reopened; renewed work requires a new request.
