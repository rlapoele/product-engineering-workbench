...
The Definition records metadata—name, optional description and declared format version—and explicit intentionally omitted sections with optional reasons. It stores typed color, typography, spacing, rounded-shape and component token collections. Colors have CSS color values; typography records font family, size, weight, line height and optional spacing or font settings; spacing and rounded shapes use dimensions or numbers as appropriate; components use named component or variant records with literal or token-reference property values. Its ordered prose guidance is Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components and Do's and Don'ts. Token values are the normative generated values, while prose gives rationale, use and guardrails. Do's and Don'ts are concise structured guidance entries.
