...
Prepare Handoff uses a scope panel over the document outline. The first handoff starts with the entire Specification as Definition Scope and enabled readiness validation and report inclusion; preparing it confirms that scope. Customization exposes explicit actions to add selected knowledge to Definition Scope or Supporting Context, mark directly related knowledge as outside the handoff, or remove it. Direct External Dependencies are derived, shown separately and cannot be relabeled as included knowledge. The panel shows deterministic minimum-definition checks per Definition Scope, labels shared evidence with the scopes it serves, does not auto-add missing knowledge, and offers a package-boundary preview before preparation. Its Design System Companion card follows readiness controls and precedes that preview. Inclusion is an explicit default-off choice. When enabled, the card exposes a profile selector with a visible default, a conditional compatible primary-color binding and compact Ready-to-include, Warning or Needs-attention preflight. It is unavailable without an included Definition, explaining how to add the Definition as Supporting Context or create it without changing scope automatically. Turning it off skips companion-profile validation but leaves ordinary selected UX/UI guidance intact. The package preview includes the chosen companion and profile/version, known warnings and a read-only generated-file preview. Other visual detail remains prototype work.
...
# UX-005 — How should UX/UI Design Requirements and Interaction Notes provide design-context handoff?
...
The Workbench needs to preserve enough explicit UX/UI knowledge to prepare a meaningful, implementation-facing `design.md` companion in an Implementation Handoff. This should constrain generic AI-generated interface output without making the product a design-system authoring or frontend implementation environment.

## Current Direction

Shared Design Guidance is structured, section-owned Product Knowledge containing a bounded Design System Definition. It gives the workbench explicit typed reusable visual data and ordered human-readable guidance, providing shared context for Screen / Views, User Flows and UI Requirements without duplicating their local obligations or inferring token values from prose.

The Definition records required Name, optional Description and explicit intentionally omitted sections with optional reasons. Name is prefilled from the Project title but remains editable; Name and Overview are the minimum saved Definition. It stores typed color, typography, spacing, rounded-shape and component token collections. Colors have CSS color literals or compatible references; typography records font family, size, weight, line height and optional spacing or font settings; spacing and rounded shapes use dimensions or numbers as appropriate. Each component has an author-defined identifier and one or more named configurations. A configuration has self-contained literal or compatible token-reference property values, plus optional description and labels or tags. It may represent any variant, interaction state, size, theme treatment or combination; no state or variant names are required or interpreted by the Workbench, and configurations do not inherit from one another. Its ordered prose guidance is Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components and Do's and Don'ts, with Overview required and the others optional. Token identifiers are author-defined: a raw palette or scale such as `blue-600`, a semantic alias such as `action-primary`, and other useful conventions are all valid. Compatible references permit semantic tokens to reuse raw values. Optional labels or tags organize a user's work but do not impose categories. Token and component identifiers are unique within their collections, configuration names are unique within their component, and references must resolve, be type-compatible with their use and be acyclic. Source validation does not require semantic naming, a foundation-versus-semantic split or another design-system methodology. A standard section must be represented by relevant typed data or prose, or be explicitly listed as intentionally omitted. Token values are the normative generated values, while prose gives rationale, use and guardrails. Do's and Don'ts are concise structured guidance entries.

The Definition is section-owned, not a collection of independent Design Token, Color, Typography or UI Component Product Artifacts; it has no visual-canvas workspace, component-inheritance graph, separate UI-state model or frontend implementation model. Resource References retain external design systems, Figma files, wireframes, prototypes and screenshots without making them canonical token data. Specific interface behavior remains a UI Requirement; measurable quality remains a Non-Functional Requirement; cross-cutting responsive or accessibility principles may remain Shared Design Guidance. Screen / Views, User Flows and UI Requirements are not design-system tokens, though a handoff may render them in a clearly labelled non-normative Workbench extension.

The UX/UI section exposes `Add Design System Definition`, which opens a Project-title-prefilled private draft with Name and required Overview. Once saved, the Definition stays inline as section-owned knowledge, with ordinary edit and remove actions rather than a separate design-system workspace. Collection-specific add actions create Colors, Typography, Spacing and Rounded shapes. Every value explicitly selects Literal or Token reference; compatible existing targets appear in the reference picker, and creating a missing target preserves then resumes the original draft. `Add Component` creates an author-defined Component with its first required named configuration; `Add Configuration` is available from saved components. Configurations have free-form property-name/value tables, with optional common-name suggestions and literal or compatible-reference values. A standard section with no relevant data or prose may be intentionally omitted with an optional reason; adding data or prose clears that omission. Source saves block only missing required source fields, duplicate identifiers, unresolved or incompatible references and reference cycles. They do not apply target-profile names, primary-color bindings, flattened-component keys or target-value syntax.

Definition edits are ordinary section Revisions. Renaming a referenced token, Component, Configuration or reference-target property preserves and updates the logical reference. Removal is available only without inbound references; otherwise the Workbench lists dependent entries and requires deliberate repointing or removal before retrying. It never cascades deletion or silently substitutes a value. Removing the Definition itself is separately confirmed and cannot delete independent Screen / Views, User Flows, UI Requirements or Resource References. These independent Product Artifacts do not become Stale when a Definition changes; later handoffs use the current revision and prepared snapshots remain immutable.

During Prepare Handoff, the author chooses whether to include a Design System Companion and selects or accepts a visible default, versioned Export Profile. The first profile, Google `DESIGN.md` `alpha`, Workbench mapping revision `1`, follows the Google Labs Code [DESIGN.md Format](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md) pinned at upstream commit `9bf8eae67128b6cc55ad9bf86665767deb4c11cd` to map Definition metadata, intentional omissions, typed token collections and ordered prose to its target format. It preserves compatible references with target `{path.to.token}` syntax; renders configuration descriptions in Components guidance; keeps labels and tags as authoring metadata; and flattens each Component/configuration pair as `component--configuration`, blocking a duplicate output key rather than altering a source identifier. If Colors are included and no source color token is named `primary`, the author explicitly selects a source token as the profile's primary-color binding; the export emits a derived `primary` alias without changing canonical data. Its recognized component properties are `backgroundColor` and `textColor` (CSS Color literal or compatible Color reference), `typography` (compatible Typography reference), and `rounded`, `padding`, `size`, `height` and `width` (target-valid Dimension literal or compatible Dimension reference). A known property that fails its target type or emitted-reference rule blocks the profile. An unknown property remains source-valid and is retained with a warning when its literal or compatible reference is representable in the target; only an unrepresentable value or unresolved emitted target reference blocks it. An absent required binding and duplicate flattened component entries also block that profile. When active Screen / Views, User Flows or UI Requirements are explicitly included in the selected package, the companion adds one `## Workbench Extension (Non-Normative)` after all standard target sections, with non-empty type subsections in Screen / View, User Flow and UI Requirement order. It uses prepared document order within each type and never infers relationship-neighbor inclusion, presents out-of-package knowledge or exposes a second scope control; Screen / View Resource References follow ordinary package inclusion. The selected inclusion decision, target profile/version, Workbench mapping revision, pinned source reference, bindings, warnings and generated companion are immutable prepared-handoff snapshot data, so the same Definition may be handed off later without a companion or with another supported profile. A later upstream-specification change cannot alter a supported profile automatically: mapping, validation or serialization behavior changes only through a newly supported mapping revision or profile, deliberately selected for a future handoff.

Each Screen / View extension entry is headed by its readable reference and Title, then gives its required Purpose and only non-empty Key content and actions, Key states and Visual references blocks. A visual reference is a labelled link to its package-relative managed file or external URL; its optional canonical Description may appear as a concise caption, and only supplied represented state or viewport context appears beside it. Managed files are copied into the handoff package and never embedded in `design.md`. The entry excludes User Profile context, Feature or Functional Requirement relationships, Acceptance Criteria, implementation routes and component structure, which remain canonical Specification material.

Each User Flow extension entry is headed by its readable reference and Title, then gives the primary User Profile name, Entry context and Intended outcome. Its **Journey steps** retain every Action, Surface, optional Relevant state and Feedback and continuation in the authored order, followed by non-empty **Intentional exits** and **Error recovery paths**. A Surface whose Screen / View is included in the package links to that extension entry. Otherwise, the step retains only plain `SV-### — Title` text marked **Outside package**; it neither links to nor adds the missing Screen / View detail. The projection excludes Feature and Functional Requirement relationships, Acceptance Criteria, User Profile detail, Screen / View detail, implementation routes and component structure.

Each UI Requirement extension entry is headed by its readable reference and Title, then gives its canonical Requirement statement. Its **Applies to** block distinguishes the primary Screen / View or User Flow target from any additional targets. Included targets link to their extension entries; absent targets remain plain `SV-### — Title` or `UF-### — Title` text marked **Outside package**. Non-empty **Interaction and state details**, **Accessibility expectations**, **Responsive expectations** and **Resource references** blocks follow in that order. Each Resource reference is a labelled managed package-relative or external link with an optional Description caption, never an embedded file. The projection excludes `depend_on` relationships, Acceptance Criteria, unrelated Feature context, implementation routes and component structure. The complete initial Workbench-extension projection is now defined.

A partial-package validation scenario confirms the projection boundary: a package includes `SV-001 Project overview`, `UF-001 Create first project` and `UIR-001 Make project creation clear`, but excludes `SV-002 Create project dialog`. `SV-001` uses its linked managed empty-overview mockup and supplied empty-state/desktop Visual Context. `UF-001` links its included overview Surface but retains the dialog Surface as plain **Outside package** text. `UIR-001` links its primary overview target, retains its additional dialog target as **Outside package**, renders applicable interaction, accessibility and responsive content, and links its external form-validation prototype. The package gains no dialog entry, User Profile detail, relationship detail, external copy or embedded asset; its extension keeps the fixed type order and prepared snapshot.

A Project Workspace validation scenario confirms that an author can save a minimum Definition, then add raw `brand-blue-600`, semantic `action-primary`, typography, rounded values and a `button` / `primary-hover` Configuration; explicitly omit Elevation & Depth; and retain an unknown `borderColor` property as source-valid knowledge. The Google profile requires the author to bind `action-primary` as its primary color, then generates the derived `primary` alias, flattened Component entry, Configuration prose, property warning and non-normative Screen / View and UI Requirement extension. The immutable snapshot retains all selected profile results. A recovery branch proves the source-versus-profile boundary: `button` / `primary--hover` and `button--primary` / `hover` remain source-valid but flatten to the same target key, blocking that export until the author changes one identifier.
...
Each UI Requirement receives an immutable project-scoped readable reference at creation, such as `UIR-001`. It does not encode its title, Screen / View, User Flow, implementation route, component or accessibility standard. It remains stable through edits, applies-to relationship changes and archival, and is retained in prepared handoff snapshots and derived `design.md` output for unambiguous traceability.
...
The UX/UI section presents UI Requirements in a fixed block. `Add UI Requirement` opens a private draft for Title, a deliberately selected primary Screen / View or User Flow target, and the Requirement statement. An optional `Also applies to` control allows authors to select additional targets during the same draft without making them a save prerequisite. Starting from a Screen / View or User Flow preselects that target visibly as the changeable primary target. If the required target does not yet exist, the workbench preserves the draft while the author explicitly creates the Screen / View or User Flow, then resumes with it selected. Saved UI Requirements use ordinary edit and locally confirmed archive actions, existing Resource Reference authoring, and direct Acceptance Criterion creation with the UI Requirement preselected as primary validation target.

Primary-target reassignment is deliberate. Promoting an existing additional target to primary retains the former primary as an additional target because the applies-to target set has not changed. Selecting a wholly new primary target requires the author to choose whether to retain the former primary as an additional target or remove it; the workbench never silently broadens or narrows applies-to scope.

When a UI Requirement changes or is archived, directly validating Acceptance Criteria become Stale. Every Screen / View or User Flow it applies to receives a coverage/readiness cue rather than becoming Stale: its independent interface context or journey may remain accurate while its documented local obligation changes or disappears. A changed or archived applies-to Screen / View or User Flow, or depended-on Functional Requirement, makes the UI Requirement Stale. Primary and additional targets behave identically for propagation. Changing or archiving a UI Requirement does not automatically make its applies-to targets or depended-on Functional Requirement Stale. A material Resource Reference change attached to the UI Requirement is its Revision and follows the same propagation.

A Create first project scenario validates the complete UX/UI chain. Shared Design Guidance supplies a calm, clear starting intent without replacing local obligations. `SV-001 Project overview` supports `FEAT-001 Manage projects` and depends on `FR-020 Make project creation available`; `UF-001 Create first project` includes the overview and create-project Screen / Views. `UIR-001 Present project creation from an empty overview` applies to `SV-001` and depends on `FR-020`; it requires the empty overview to present a clear, programmatically named Create project action. `UIR-002 Make project-creation validation recovery clear` applies to `UF-001` and records known correction guidance and retry after validation failure without requiring a redundant Functional Requirement link. Direct Acceptance Criteria may validate the Feature, Requirement, Screen / View, Flow and either UI Requirement only where their observable evidence genuinely covers each target. Changing `UIR-001` stales its direct evidence and cues `SV-001` without staling `FR-020` or `UF-001`; changing `SV-001` stales `UIR-001` and `UF-001`; changing `FR-020` stales the artifacts that depend on it; and changing `UIR-002` stales its direct evidence and cues `UF-001` without staling the Flow itself.
...
The bounded Design System Definition, optional versioned Design System Companion and complete initial Workbench-extension projection are sufficient for the current scope. The companion is included only by explicit author choice during Prepare Handoff, and unavailable or incomplete context remains honestly represented through the selected package boundary, profile preflight and ordinary Specification material rather than invented in `design.md`.

## Deferred Detail

Richer Markdown presentation controls, a visual design workspace, component inheritance or a separate UI-state model are not required. Additional target formats and Google mapping revisions require a concrete handoff need that the established typed source model, supported profile evolution and linked Resource References cannot express clearly. Upstream changes do not themselves create a requirement or alter the current profile.
...
Project creation follows a short ordered setup: the owner first selects Greenfield or Brownfield, then supplies Content language, Project title and short description. There is no separate Intended Specification Purpose field. Greenfield optionally asks no more than four skippable, plain-language questions about the problem or opportunity, intended outcome, target users and known uncertainty. These temporary **Project-start Notes** may recommend a preset and relevant sections; after creation, the owner can explicitly use each answer as editable, unsaved first-draft material for one suggested destination. They are neither Sources nor canonical Product Knowledge, and cancelling creation retains neither a Project nor Project-start Notes. Brownfield previews its immediate post-creation Source actions rather than collecting or uploading Sources before the Project and attachment scope exist.

The owner then explicitly selects the Specification Document Template, followed by preset and section customization, before choosing Create Project. The MVP's sole available template is preselected as currently available, not falsely presented as a personalized recommendation. The interface still reserves the recommendation and rationale pattern for a future multiple-template choice, while never overriding the owner's final template selection.
