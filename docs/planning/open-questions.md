...
- A newly captured Product Artifact is placed at the end of the deterministic canonical section for its type, without a placement picker. An unselected canonical section requires an explicit add-section or cancel choice; section-local reordering is a separate document-composition Revision and does not change Conversation provenance or Outcome Links.
- The remaining question is how the owner-facing destination choices, draft review, canonical placement and resulting outcome links should be validated as one coherent journey.
