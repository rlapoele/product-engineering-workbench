...
- The selected first slice now has a low-fidelity UX prototype and interaction-validation model: five observed moderated sessions plus a keyboard/screen-reader-oriented pass test the first-use home, fixed-starter Project creation including Content Language, empty document, local Goal creation, explicit save, later Project return and essential pending, error and keyboard states. The evidence rubric treats false saved-state belief, unsafe recovery and essential keyboard blocks as critical; prototype evidence remains a prerequisite to technology selection. Browser-locale detection, interface-language settings, visual design, authentication mechanics and broader MVP capability remain deferred.
...
- Build and run the approved low-fidelity prototype exercise, then evaluate its evidence before opening technology selection.
