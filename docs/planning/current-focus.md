...
- `Core Features` now uses independently versioned Features with a title, concise description and explicit User Need links. Optional section-owned Feature Groups organize document presentation without introducing technical-module semantics, while contextual related-knowledge creation preserves drafts and preselects new prerequisite relationships.
- `User Stories and Use Cases` now distinguishes concise Content-Locale-rendered User Stories from structured interaction Use Cases. Both require a primary Core Feature and use the same preserved-draft prerequisite path; User Stories remain anchored to a primary profile, while Use Cases support controlled main and alternate flows.
- Content Locale now explicitly governs fully localized document templates: language-neutral section structure and semantic text keys resolve every section title, guidance, placeholder and fixed wording through template-supported locale resources. Interface Locale remains responsible for workbench controls and validation; authored content is never automatically translated and fallback language is always explicit.
- `Acceptance Criteria` now uses independent validation artifacts with either direct observable pass conditions or structured Given/When/Then scenarios. One primary and optional additional explicit targets let the same criterion render contextually beside a Feature, Story, Use Case, Requirement or UX artifact without duplication.
- Functional Requirement references are now bounded for durable specification use: each requirement receives an immutable project-scoped readable reference such as `FR-001`, while familiar hierarchical outline numbering remains derived presentation that may change as the document evolves. Prepared handoffs retain the rendered outline and included requirement revision snapshots without changing earlier references or history.
- Functional Requirements may now use optional one-level Requirement Groups with a title and short purpose. These are section-owned presentation headings, not technical modules or artifacts; grouping and order affect only document composition and derived outline numbering.
- Functional Requirement leaves now have a required primary Core Feature, title and independently testable statement, with optional conditions only for the same behavior. A distinct or cross-cutting obligation becomes another requirement; missing Acceptance Criteria remains a visible coverage cue rather than an initial-save blocker, and shared evidence is valid only when it demonstrably covers each linked requirement.
- Functional Requirement qualifiers now use one flat `Conditions and constraints` list: authors write plain-language entries without choosing or receiving a Condition-versus-Constraint classification, while distinct obligations still become separate requirements.
- Functional Requirements retain one primary Core Feature as their authoring anchor while allowing additional explicit Feature links when one behavior genuinely supports multiple capabilities. Shared wording or Requirement Group membership never infers those relationships.
- Functional Requirements now has a contextual authoring path: section actions add a requirement, group or Core Feature; group-local creation pre-associates a requirement; and a missing prerequisite Feature is created through the canonical related-knowledge path with the requirement draft preserved and resumed. Saved requirements provide ordinary edit/archive controls plus target-preselected Acceptance Criterion creation.
- Non-Functional Requirements are now bounded as independent, explicitly scoped quality artifacts: each records a category, measurable statement and one or more targets, including whole-Specification scope for product-wide expectations. This separates cross-cutting qualities from Functional Requirement behavior and local qualifiers.
- Non-Functional Requirement quality categories now use a compact fixed initial vocabulary—Performance, Reliability and availability, Security, Privacy, Accessibility, Compatibility, Usability, Compliance and Other with a required custom label. Categories are localized organizational and review aids, not workflows, modules or document subsections.
- Non-Functional Requirements now use one required primary scope target and optional additional explicit targets. The initial target set is Whole Specification, Core Feature, Functional Requirement, Use Case, Screen / View, User Flow and UI Requirement, keeping quality traceability anchored to behavior and interaction rather than abstract goals or document groups.
- Non-Functional Requirements now receive immutable project-scoped readable references such as `NFR-001`. These remain stable across category or document changes and are retained in prepared handoff snapshots, alongside their internal artifact identity.
- Non-Functional Requirement leaves now pair a concise title with a canonical measurable statement. Optional Measurement context clarifies events, conditions, baselines or sources without substituting for explicit Acceptance Criteria or a test plan.
- Non-Functional Requirements now reuse the ordinary Acceptance Criteria model: they are eligible validation targets, provide a non-blocking coverage cue when unlinked, and can start target-preselected criterion authoring. A shared criterion remains valid only where its observable evidence covers every linked Functional or Non-Functional Requirement.
- Non-Functional Requirements now have a direct document-first authoring flow: one section action, deliberately selected primary scope, preselection when started from an eligible target, ordinary edit/archive controls and no category-based grouping layer.
- The `Data or Domain Model` direction is now established: Domain Concepts and Domain Rules are independent Product Artifacts. A Domain Rule is either a Business Rule or an Invariant, enabling direct traceability from domain structure and governing policy to related Functional Requirements and Acceptance Criteria without prescribing database design.
- Domain Concepts now have a lean initial shape: required title and plain-language definition, plus an optional `Key business attributes` list limited to business meaning. Important states may remain in that list until structured state modeling is justified; schema-level details remain outside the section.
- Domain Concepts now receive immutable project-scoped readable references such as `DC-001`, which survive renaming, archival and saved handoff snapshots.
- Domain Relationships are now structured first-class Artifact Relationship records between Domain Concepts, not separate Product Artifacts. They require source, readable statement and target, and support optional cardinality at either end without prescribing database relationships.
- Domain Rules now have a minimal formal shape: required title, Business Rule or Invariant kind, canonical statement and explicit governing Domain Concept or Domain Relationship scope. Conditions and exceptions stay in the canonical statement rather than a separate field.
- Domain Rules now receive immutable project-scoped readable references such as `DR-001`, independent of kind and document placement and retained after archival and in handoff snapshots.
- Functional Requirements may now explicitly `enforce` Domain Rules, distinguishing the system obligation that upholds a governing policy from provenance or dependency relationships.
- Domain Rules are now eligible Acceptance Criterion targets with non-blocking coverage cues and target-preselected authoring. Shared evidence may validate both a Rule and related Functional Requirement only when it genuinely covers both; Concepts and Relationships remain structural context.
- The `Data or Domain Model` now has a direct document-first authoring flow: fixed Concept, Relationship and Rule blocks; three section actions; contextual preselection; and preserved drafts during explicit prerequisite creation. Concepts and Rules use ordinary edit/archive actions, while Relationship records use edit/remove actions.
- Data-model impact behavior now follows ordinary Stale-versus-coverage semantics: changed Rules make enforcing Requirements and direct validation evidence Stale, while changed Requirements create Rule coverage cues. Changed Concepts or Relationships make their governing Rules Stale; connected Relationship records receive review cues rather than lifecycle states.
- Domain Rules now explicitly `govern` their Concepts or Relationships. The required document-facing Governs list is a projection of these graph relationships, preserving one canonical traceability record.
- A concrete subscription scenario validates the integrated Data or Domain Model chain: Domain Concepts and Relationships provide structure; a Domain Rule governs it; a Functional Requirement enforces the Rule; and shared Acceptance Criteria validate both, with the selected propagation semantics distinguishing Stale from coverage cues.
- The Data or Domain Model is now resolved for the current scope: its structured Relationship record and `governs`, `enforces` and `validates` relationships provide sufficient traceability. Richer relationship vocabulary, relationship references and structured state modeling remain deferred until a concrete need justifies them.
- UX/UI discovery now establishes Shared Design Guidance as structured, section-owned knowledge containing a bounded Design System Definition. It provides canonical machine-readable token and human-readable guidance inputs for a derived `design.md` companion without adding a visual design workspace or independent design-system Product Artifacts.
- The Design System Definition now has an initial export-oriented shape: metadata and explicit omissions; typed colors, typography, spacing, rounded-shape and component token collections; and ordered Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components and Do's and Don'ts guidance. Tokens are explicit data rather than values inferred from prose.
- Design System Definition now has a minimum saved shape: editable Project-title-prefilled Name and required Overview; optional Description; explicit omissions; unique token and component identifiers plus component-local configuration names; and typed collection fields aligned to generated `design.md` token groups. Export targets and versions are not canonical Definition metadata.
- Design System Definition token identifiers are author-defined rather than semantically prescribed: raw palette or scale tokens, semantic aliases and other conventions are valid across token collections. Compatible references may build one token from another; optional labels or tags organize without classifying. Source validation is limited to uniqueness, resolving, type-compatible and acyclic references—not a required token taxonomy or methodology.
- The Design System Definition component model is now explicit: each author-defined component has one or more self-contained named configurations with literal or compatible token-reference properties and optional descriptive labels. A configuration may represent any variant, state, size, theme treatment or combination, without a required state list, parsed naming convention or component-inheritance graph.
- A Design System Companion is optional at Prepare Handoff, where the author selects or accepts a default, versioned Export Profile. The initial Google `DESIGN.md` `alpha` profile deterministically maps source tokens and prose, flattens component configurations, preserves compatible references, adds an explicit primary-color binding when needed and snapshots the selected profile, bindings and generated companion. Screen / Views, User Flows and UI Requirements remain canonical Product Artifacts and appear only in a clearly labelled non-normative Workbench extension; Resource References remain external supporting sources.
- Design System Definition authoring is now direct and document-first: a Project-title-prefilled private Definition draft saves with Name and Overview, then inline collection, component/configuration, prose and omission actions support incremental work. Literal-versus-reference selection, compatible reference picking and preserved prerequisite creation keep source knowledge structurally valid; source validation is distinct from profile-specific handoff validation.
- A Project Workspace scenario now validates the Design System Companion end to end: flexible raw and semantic token names, explicit omissions, component configuration export, primary-color binding, unknown-property warning, Workbench extension and immutable handoff snapshot. A flattened-key collision recovery confirms that source-valid user names can still require a selected export-profile correction.
- Prepare Handoff now has a scoped Design System Companion card: explicit default-off inclusion, visible default profile, conditional compatible primary-color binding and compact Ready/Warning/Needs-attention preflight before the package preview. A missing included Definition offers ordinary scope or creation actions without auto-changing the package; the preview and snapshot make the output choice visible.
- Definition edits are reference-safe section revisions: renames retain logical references, while removal is blocked by inbound references until the author repoints or removes them. No cascade, substitute or automatic Stale propagation reaches independent Product Artifacts; future handoffs use the revision and past snapshots remain immutable.
- Google `DESIGN.md` `alpha` component-property conformance is now explicit: the profile recognizes its standard Color, Typography-reference and Dimension properties; known type or reference failures block that export, while source-valid unknown properties pass through with a warning when representable. This constrains only the selected profile, not the Definition's free-form source property table.
- Design System Companion Export Profiles are now reproducible rather than live external dependencies: the initial Google `DESIGN.md` `alpha` profile records Workbench mapping revision `1` and a pinned upstream source revision. Any later mapping, validation or serialization change requires a new selectable mapping revision or profile, while existing prepared snapshots remain unchanged.
- The `design.md` Workbench extension now has a bounded package projection: it is present only for active included UX/UI artifacts, follows the standard target sections, orders Screen / Views, User Flows and UI Requirements deterministically, and never follows relationships to add knowledge or create another scope selector.
- Screen / View entries in the Workbench extension now have a concise handoff projection: readable reference and title, Purpose, non-empty key-content, key-state and visual-reference blocks, and labelled package-relative or external visual links with only supplied reference-local context. Managed assets remain package files, not embedded Markdown content; relationships, evidence and implementation structure remain in canonical Specification material.
- User Flow entries in the Workbench extension now preserve their journey without expanding scope: readable reference/title, user, entry, outcome, ordered action/surface/state/feedback steps and non-empty exit or recovery paths. Included Screen / View surfaces link locally; otherwise their `SV-###` reference and title remain plainly marked `Outside package`, with no imported view detail.
- UI Requirement entries now complete the Workbench extension projection: readable reference/title and canonical statement; primary and additional applies-to targets; non-empty interaction, accessibility, responsive and resource-reference blocks. Targets link only when included, otherwise stay marked `Outside package`; attached resources remain labelled package-relative or external links without embedded files, while dependencies, evidence and implementation detail remain canonical Specification content.
- A partial-package companion scenario now validates the complete Workbench extension: included Screen / View, User Flow and UI Requirement entries link only in-package context; required missing Screen / View references remain visible as `Outside package`; managed and external resources follow their established link/copy rules; and no relationship traversal or file embedding expands the handoff.
- The Design System Definition and initial Google `DESIGN.md` companion are now closed for the current scope. Presentation refinements, source-model expansion, additional target formats and mapping revisions are deferred until a concrete handoff need cannot be represented by the established model, supported profile evolution or linked Resource References.
- Screen / View is now established as an independent Product Artifact: a stable user-facing interface boundary for related flows, UI requirements, visual references and handoff context, without prescribing implementation routes or frontend architecture.
- Screen / View now has a lean initial shape: required Title and Purpose, plus optional key content/actions, user-facing states, relevant User Profile context and visual references. It remains distinct from UI Requirements' local obligations and User Flows' cross-view journeys.
- Screen / View now has a small explicit relationship set: it may support a Core Feature, depend on a Functional Requirement and be directly validated by Acceptance Criteria; User Flows may include it through a non-containment relationship carrying flow-specific context when needed.
- Screen / View now has direct document-first authoring: its own section block and private draft, contextual Feature or Requirement relationship preselection, ordinary edit/archive and direct reference or acceptance-coverage actions, with no forced Flow or visual-reference prerequisite.
- Screen / Views now receive immutable project-scoped `SV-001` references. These remain stable through title, placement, relationship and archival changes, and are retained in handoff snapshots and derived design-context output without implying routes or URLs.
- Screen / View propagation now follows Stale-versus-coverage semantics: view changes make including Flows and direct validation evidence Stale, while supported Features receive coverage cues; upstream Feature or Requirement changes make affected Views Stale, and Flow changes do not automatically invalidate their included Views.
- A concrete Project overview scenario validates Screen / View in isolation: purpose, content/actions, empty state, visual reference, Feature support, Requirement dependency and observable Acceptance Criterion form a coherent chain without requiring a route, component tree or prematurely defined User Flow.
- User Flow is now established as an independent Product Artifact for cross-view user-experience journeys. It owns entry, decisions, feedback, recovery and outcome, explicitly includes Screen / Views, and remains distinct from Use Cases and individual interface contexts.
- User Flow now has a structured initial shape: required user, entry, outcome and ordered Screen / View-linked Journey steps; optional relevant state; and explicitly distinct intentional-exit versus error-recovery paths. It captures known visible experience without inventing technical edge cases.
- User Flow relationships now reuse the established graph: step-owned `includes` links to Screen / Views; optional Feature support and Requirement dependency; and direct Acceptance Criterion validation. User Story and Use Case context stays ordinary `relates_to`; UI Requirement semantics remain deferred.
- User Flow now has direct document-first authoring: its own section block, preserved prerequisite creation for its primary User Profile and first Screen / View, ordered step editing, contextual relationship preselection, and ordinary edit/archive or acceptance-coverage actions.
- User Flows now receive immutable project-scoped `UF-001` references. These survive title, step-order, relationship and archival changes, and provide durable handoff, UX-review and design-context traceability without implying routes, analytics or tests.
- User Flow propagation now follows Stale-versus-coverage semantics: Flow changes make direct validation evidence Stale and give supported Features coverage cues; changed Profile, View, Feature or Requirement context makes the Flow Stale, while a Flow change does not invalidate those independently meaningful artifacts.
- A concrete Create first project scenario validates User Flow end to end: Screen / View-linked steps, user action, state and feedback, distinct intentional exit and error recovery, behavior traceability and selected propagation rules form a coherent journey model without becoming a route map or test script.
- UI Requirement is now established as an independent Product Artifact for one observable interface obligation. It bridges system behavior and user-facing experience while remaining distinct from shared design direction, interface contexts, journeys and frontend implementation choices.
- UI Requirement now has a lean initial shape: required title, primary Screen / View or User Flow target and canonical statement; optional additional targets and local interaction/state, accessibility, responsive or reference detail. One independently testable obligation stays one requirement.
- UI Requirement traceability now uses explicit `applies_to` relationships to Screen / Views or User Flows, with primary/additional target metadata; optional Functional Requirement dependency and direct Acceptance Criterion validation reuse established graph semantics without redundant feature or design-guidance links.
- UI Requirements now have a direct document-first authoring flow: a fixed section block, an explicitly chosen visible primary target, optional in-draft additional targets, preserved Screen / View or User Flow prerequisite creation, ordinary edit/archive and target-preselected acceptance authoring. Reassigning primary scope retains an already related target but otherwise requires an explicit retain-or-remove choice for the former primary.
- UI Requirements now receive immutable project-scoped readable references such as `UIR-001`. These remain stable across edits, target changes and archival, and are retained in handoff snapshots and derived design-context output without encoding UI implementation choices or accessibility standards.
- UI Requirement propagation now follows Stale-versus-coverage semantics: requirement changes make direct validation evidence Stale and give every applies-to Screen / View or User Flow a coverage cue; changed targets or depended-on Functional Requirements make the UI Requirement Stale, without invalidating independently meaningful targets or behavior from a local obligation change.
- A Create first project scenario now validates the complete UX/UI chain: shared guidance, Screen / Views, User Flow, Screen/View- and Flow-scoped UI Requirements, underlying Functional Requirement and direct acceptance evidence remain distinct, share evidence only where observable coverage genuinely overlaps, and apply the selected propagation rules without redundant links or implementation prescription.
