...
- The standalone Collaboration Request lifecycle is now bounded: a required Request Brief and explicit Draft, Submitted, In Progress, Responded, Declined and Cancelled states preserve one bounded response cycle; Reopen returns terminal requests to a private Draft where only the brief may change before resubmission, while its selected human recipient retains a frozen read-only view of prior shared evidence.
