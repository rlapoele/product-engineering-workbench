...
- UX/UI discovery now establishes Shared Design Guidance as structured, section-owned knowledge. It may preserve reusable visual direction and known named values or conventions, and provide canonical inputs for a derived `design.md` handoff companion without becoming design-system management.
- Shared Design Guidance now has a lean three-block shape: required Design direction, optional Reusable design conventions with precise named values or rules and intended use, and optional Application guidance. Its fixed convention headings support readable authoring and derived `design.md` structure without requiring authors to fabricate design values.
- Shared Design Guidance now has direct document-first authoring: fixed visible blocks, independent starting actions, and preserved drafts when a missing required Design direction must be explicitly created. Conventions are structured section content with edit/remove controls; no new artifacts or management workspace are introduced.
