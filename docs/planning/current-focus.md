...
- Design System Definition now has a minimum saved shape: editable Project-title-prefilled Name and required Overview; optional Description; visible system-managed output format/version; explicit omissions; unique token and component identifiers plus component-local configuration names; and typed collection fields aligned to generated `design.md` token groups. Prepared handoff snapshots retain the generated version.
- Design System Definition token identifiers are author-defined rather than semantically prescribed: raw palette or scale tokens, semantic aliases and other conventions are valid across token collections. Compatible references may build one token from another; optional labels or tags organize without classifying. Validation is limited to uniqueness, resolving, type-compatible, acyclic references and target-format validity—not a required token taxonomy or methodology.
...
- Screen / Views, User Flows and UI Requirements remain canonical Product Artifacts rather than tokens. A `design.md` may include them only as a clearly labelled Workbench extension, while Resource References retain external design sources and the detailed Definition authoring and validation flow remains to be decided.
