...
- The first slice now has an implementation-authorization model. No repository product code begins until a local-only disposable Dependency Verification Record establishes one exact runtime/package set, reviewed Better Auth `auth`-schema DDL and toolchain compatibility, and the Project Owner explicitly authorizes the named slice. This platform lock neither deploys to Railway nor configures OAuth or uses Product Knowledge. Railway migration gating, staging OAuth, recovery, manual accessibility and production checks remain later release-readiness gates.
