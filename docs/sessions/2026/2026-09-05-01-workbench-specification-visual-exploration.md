...
With typography held constant, the owner found the blue palette visually stronger on screen than ochre despite personally liking ochre. The current blue accent still reads too close to the blue-tinted primary ink. Measurement suggests testing accent chroma `0.15` at the existing `46%` lightness and `255` hue: it remains within sRGB and preserves strong contrast, whereas `0.16` begins leaving the gamut. This is the next comparison candidate, not a selected production token or final palette decision.
