...
A subsequent annotation found that the Edit action's larger touch target placed it visually below the requirement reference and title even though their top edges matched. The local header row now aligns the action to the requirement heading's first-line baseline, including when the title wraps, without reducing the action's usable target or adding a fragile positional offset.

The owner then selected Urbanist with `0.035em` tracking and the blue palette with `0.15` accent chroma as the prototype's current baseline, while keeping the comparison controls and production decision open. The sidebar navigation was also corrected to use nested lists and anchors for destinations and document locations; separate buttons now own only the Specification and requirement-group disclosures. Top-level icon-and-label links use `gap-3`, and navigation-item surfaces remain square while buttons retain their own component shape.
