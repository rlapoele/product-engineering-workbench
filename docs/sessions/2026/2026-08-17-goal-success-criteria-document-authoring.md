...
- The section has no section-level introductory prose. It continues to offer `Add Goal` after Goals exist. A Goal exposes `Edit Goal`, secondary `More actions` including `Archive Goal`, and a separate child-area `Add Success Criterion` action.
