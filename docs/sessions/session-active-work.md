...
- The selected slice's low-fidelity UX prototype is now executable: five observed moderated first-time-user sessions plus a keyboard/screen-reader-oriented pass cover the no-Projects home, fixed-starter Project creation including editable Content Language, missing-title validation, injected creation/save pending and failure recovery, empty Specification, local `Add Goal`, inline title/content editing, explicit `Done editing`, saved Draft Goal and return to Project. The evidence rubric classifies false saved-state belief, unsafe recovery and essential keyboard blocks as critical; technology selection requires independent core completion by at least four of five participants, no retained false saved-state belief and no unmitigated critical accessibility issue. Browser-locale detection, interface-language settings, visual styling, authentication mechanics, offline behavior, collaboration, AI and handoff remain outside the exercise.
...
- The five selected first-slice packages are coherent but remain inactive planning: they describe the same authenticated-owner-to-saved-Goal journey. Before any implementation decision, they still require the evidence from the approved prototype exercise and concrete technology/operational choices that meet the agreed quality gates. A first Goal save should show ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
