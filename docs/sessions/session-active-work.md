...
- The five selected first-slice packages are coherent but remain inactive planning: they describe the same authenticated-owner-to-saved-Goal journey. Before any implementation decision, they still require a `contentLocale` default, a fixed-starter version convention, executed prototype evidence and concrete technology/operational choices that meet the agreed quality gates. A first Goal save should show ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
