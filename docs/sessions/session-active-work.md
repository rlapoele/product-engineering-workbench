...
- Request detail is one newest-first Request Cycle structure. The current cycle stays expanded with immutable recorded context and either its submitted response with labelled handling status or a compact state; private response drafts remain private. Earlier cycles are collapsed summaries; expanding one immediately shows its summary and loads its stored immutable Brief, context and response inline with loading, retry and current-detail-session caching. Detail shows only concise lifecycle milestones—submission, start, response, decline, cancellation, On Hold, resume, reopen and response-handling changes—rather than an activity feed or chat surface.
