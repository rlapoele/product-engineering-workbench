...
- The Project Collaboration Requests workspace has one role-aware, paginated list: **Requests you created** for the Project Owner, including private Drafts, and **Requests for you** for a Project Collaborator selected as a human recipient. Its default has no lifecycle or response-handling filter and uses **Last updated — newest first**. Last updated changes only for a Brief edit, submission, lifecycle transition including AI On Hold or resume, response submission or response-handling change; opening or viewing does not change it. Search covers the Brief, type, scoped-source label and participant names; filters are Request status, response handling, type and scope kind, plus recipient for the owner; sorts are Last updated newest or oldest first, creation newest first and Brief A–Z. Rows show the Brief, type, scoped source, other party, labelled Request status and concise last-update event. Contribution Responses stay inside their parent request; current-cycle response handling is a labelled request field, while a retained response after Reopen is explicitly labelled **Previous response**. Details retain the full Brief, immutable recorded context, lifecycle history, response content and handling actions. The workspace has no priority, due date, assignee, lifecycle group or needs-action worklist.
