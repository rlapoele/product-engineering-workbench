...
- The selected slice's data contract persists Project, Specification, Goal and Revision records. Project creation materializes the fixed empty Specification without a Revision and atomically persists the owner-editable Project `contentLocale`, pre-filled from the resolved Interface Locale. A private browser Goal draft becomes canonical only through `Done editing`, which atomically creates a Draft Goal and its immutable complete Revision 1. Each write carries an Operation ID; exact retries return the recorded outcome without duplication, while mismatched reuse is rejected. Relationships, later revisions and diffs, draft resumption, archiving, propagation, collaboration and sync conflicts remain deferred.
- The selected slice's low-fidelity UX prototype validates the first-use home, fixed-starter Project creation including editable Content Language, empty Specification, local `Add Goal`, inline title/content editing, explicit `Done editing`, saved Draft Goal and return to Project. It includes creation/save pending and failure recovery, accessible keyboard/focus states and non-color status/error communication. It tests save-failure draft preservation but not navigation-persistent draft resumption, browser-locale detection, interface-language settings, visual styling, authentication mechanics, offline behavior, collaboration, AI or handoff.
...
- The five selected first-slice packages are coherent but remain inactive planning: they describe the same authenticated-owner-to-saved-Goal journey. Before any implementation decision, they still require a fixed-starter version convention, executed prototype evidence and concrete technology/operational choices that meet the agreed quality gates. A first Goal save should show ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
