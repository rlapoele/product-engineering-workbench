...
- Response-to-Revision traceability is intentionally narrow: only an authorized human's ordinary save may explicitly link one whole Contribution Response to the resulting Revision. The optional Response Source Link preserves response-cycle navigation and known provenance without a separate workflow, item-level disposition, automatic acceptance or exact authorship claim; acknowledgement, dismissal and Acted On never create it.
- Context Relevance is a request-specific inclusion decision, not a score, graph-distance rule or instruction to include every related item. Ordinary assembly contains only Primary and Supporting context; Optional material requires an explicit Brief need or visible contributor expansion, while non-supporting nearby material remains excluded. Inferred context cannot fill missing Primary evidence, and the Context Explanation names material inclusion, expansion, exclusions and limits without an exhaustive candidate inventory.
