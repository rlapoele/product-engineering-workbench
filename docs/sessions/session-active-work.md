...
- The Project Collaboration Requests workspace has one role-aware, paginated list: **Requests you created** for the Project Owner, including private Drafts, and **Requests for you** for a Project Collaborator selected as a human recipient. Search, filters and sorting replace status-specific lists and board columns. Contribution Responses stay inside their parent request; response handling status is a labelled request field, not a separate list, group or workspace.
