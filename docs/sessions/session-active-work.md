...
- The selected slice uses one deployable online modular monolith with five logical boundaries: browser presentation, identity, server application commands, canonical persistence and the fixed starter definition. The browser renders onboarding, Project and document interactions through its Project data/command boundary but never authorizes ownership or becomes canonical state. The server enforces owner-only access and atomically creates the Project/default Specification or saves the Draft Goal/first Revision. Frameworks, API style, database and deployment choices remain deferred.
- The selected slice's data contract persists Project, Specification, Goal and Revision records. Project creation server-selects and records fixed starter `implementation-ready-web-app-specification.standard-web-app` at immutable version `1`, materializes the empty Specification without a Revision and atomically persists the owner-editable Project `contentLocale`, pre-filled from the resolved Interface Locale. A private browser Goal draft becomes canonical only through `Done editing`, which atomically creates a Draft Goal and its immutable complete Revision 1. Each write carries an Operation ID; exact retries return the recorded outcome without duplication, while mismatched reuse is rejected. Relationships, later revisions and diffs, draft resumption, archiving, propagation, collaboration and sync conflicts remain deferred.
- The selected slice's low-fidelity UX prototype is executable. Before technology selection, its Project Owner runs and records a solo Prototype Review across the no-Projects home, fixed-starter Project creation including editable Content Language, missing-title validation, injected creation/save pending and failure recovery, empty Specification, local `Add Goal`, inline title/content editing, explicit `Done editing`, saved Draft Goal, Project return, declared layout/UI patterns, keyboard operation and preliminary screen-reader semantics where meaningful. One Draft-Goal-path walkthrough has found every tested interaction valid and intentionally made no visual/UI assessment. The review is a coherence and direction check, not independent usability research or accessibility conformance; no unresolved critical contradiction, false saved-state presentation, unsafe recovery path or essential keyboard blocker may remain. Independent first-time-user and broader accessibility input are execution-stage recommendations for the working product. Browser-locale detection, interface-language settings, visual styling, authentication mechanics, offline behavior, collaboration, AI and handoff remain outside the exercise.
...
- The five selected first-slice packages are coherent but remain inactive planning: they describe the same authenticated-owner-to-saved-Goal journey. The solo Prototype Review is the proportionate pre-technology evidence gate; it supports coherent interaction direction but does not claim independent usability or accessibility conformance. Before any implementation decision, the completed review and concrete technology/operational choices must meet the agreed quality gates. A first Goal save should show ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
