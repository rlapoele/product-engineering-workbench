...
- The first slice now has a strict implementation-authorization gate. A disposable local-only Dependency Verification Exercise first records exact runtime/package compatibility, Astro/React build behavior, generated Better Auth `auth` schema DDL, direct `pg`/`node-pg-migrate` behavior and the selected test tooling. It may use local packages and Testcontainers but adds no repository product code or migrations and performs no Railway deployment, OAuth configuration or real-data use. Only its accepted Record plus explicit owner authorization permits first-slice code; all Railway, staging OAuth, recovery, accessibility and production gates remain release readiness.
