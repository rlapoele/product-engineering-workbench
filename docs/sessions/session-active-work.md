...
- `Review` is a scoped, composite assessment with one visible, confirmed Review Lens per request. Whole-Specification, eligible Section and active Product Artifact scopes yield lens-qualified coverage findings only; multiple checks stay within that lens, different lenses require separate requests, and the result never approves, certifies or becomes a Readiness Result. The Readiness Fact Check Set remains exclusive to `Validate Readiness`.
