...
- Railway migration/release gating, staging OAuth verification, restoration exercises and manual VoiceOver/Safari validation remain later MVP release-readiness work. Manual accessibility validation requires its human-followable scenario template before it is performed; automated checks do not claim conformance.
...
- Implementation entry guidance is retained as a historical planning record: its five decision packages informed the completed narrow first vertical slice. It creates no roadmap, task list or authorization for a subsequent increment.
...
- The selected slice's engineering-quality baseline requires evidence across five gates before real user content: automated command correctness; owner-only authorization and privacy-safe handling; browser accessibility journey validation; privacy-safe observability plus tested restoration; and automated/recovery-checked release discipline. Automated results live as committed suite and source-revision evidence records. Manual VoiceOver/Safari validation is planned as broader MVP work progresses, when a human-followable scenario template will be created before execution. Product knowledge is confidential by default; enterprise availability, performance, compliance and penetration-testing commitments remain deferred.
...
- The first-slice verification and evidence plan is now selected. `astro check` and production build verify static/build integrity; Vitest, React Testing Library and user-event cover pure and React-Island behavior; Testcontainers PostgreSQL with committed migrations plus a separate Better Auth test-only factory cover real server/database commands without a production auth bypass; and Playwright with axe scans covers the built Node application. `npm run verify:release` combines these static, build, Vitest and Playwright checks for a local release-verification run without adding CI or deployment behavior. Chromium is the required automated browser, Firefox and WebKit are release-candidate smoke coverage, and failure artifacts may contain synthetic local/staging data only. The committed suite and source-revision evidence records retain the automated result. Manual VoiceOver/Safari validation and its human-followable scenario template are planned as broader MVP work progresses. Dedicated synthetic staging accounts exercise real OAuth before first production release and after identity configuration changes.
- The recovery-and-observability implementation is now bounded and verified: after an authoritative Project command completes, the Project module records only command category, success/failure outcome, duration and source revision through a non-authoritative Port. A PostgreSQL `ops` adapter upserts the content-free hourly aggregate, while the Pino adapter emits only event, command category, outcome, duration, random correlation ID, environment and source revision. A failed recorder cannot alter the returned canonical command outcome, and independent Pino/ledger sinks are isolated from each other. Unit and disposable-PostgreSQL integration tests prove those constraints; generated diagnostics remain synthetic-only.
...
- The first-slice executable specification was fixed before implementation and governs the completed code. `src/modules` holds Project/Fixed Starter public contracts and policies; `src/adapters` holds PostgreSQL, Better Auth, operations and browser HTTP transport; `src/composition` is the only configuration/wiring boundary; Astro `src/pages` and React `src/islands` remain thin presentation adapters; and `migrations` stays separate. It defines the four Project operations, server-derived fields, separate runtime/migration configuration, import-review rules and a staged implementation order through release evidence.
- The accepted local-only Dependency Verification Record fixes the Node `24.14.1` platform lock and exact compatible Astro/React, Better Auth/PostgreSQL and verification-tool package set. It records the TypeScript `7` to `6.0.3` replacement required by `@astrojs/check`, version-aligned Better Auth DDL generation and application through `node-pg-migrate` into the `auth` schema, and a passing Testcontainers PostgreSQL plus Playwright/axe exercise. The generator CLI remains development-only because its isolated tooling graph has advisory findings. The Record and explicit owner authorization enabled the completed ADR-026 implementation; Railway, staging OAuth, recovery, accessibility and production gates remain later release readiness.
...
- The five selected first-slice packages are coherent, and the authorized ADR-026 authenticated-owner-to-saved-Goal slice is now complete. The completed solo Prototype Review supplies the proportionate interaction evidence gate; it supports coherent direction but does not claim independent usability or accessibility conformance. Railway migration gating, staging OAuth, recovery and production checks remain later release-readiness gates; manual VoiceOver/Safari validation is planned as broader MVP work progresses. A first Goal save shows ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
