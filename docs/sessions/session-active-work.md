...
- The selected slice's low-fidelity UX prototype is executable. Its approved exercise calls for five observed moderated first-time-user sessions plus a keyboard/screen-reader-oriented pass covering the no-Projects home, fixed-starter Project creation including editable Content Language, missing-title validation, injected creation/save pending and failure recovery, empty Specification, local `Add Goal`, inline title/content editing, explicit `Done editing`, saved Draft Goal and return to Project. One solo Project-Owner walkthrough has found every tested Draft-Goal-path interaction valid and intentionally made no visual/UI assessment; it is indicative only. The evidence rubric classifies false saved-state belief, unsafe recovery and essential keyboard blocks as critical; technology selection requires independent core completion by at least four of five participants, no retained false saved-state belief and no unmitigated critical accessibility issue. Browser-locale detection, interface-language settings, visual styling, authentication mechanics, offline behavior, collaboration, AI and handoff remain outside the exercise.
...
- The five selected first-slice packages are coherent but remain inactive planning: they describe the same authenticated-owner-to-saved-Goal journey. The indicative solo prototype walkthrough supports retaining the interaction model but does not meet the approved independent-participant or accessibility evidence threshold. Before any implementation decision, the remaining evidence approach and concrete technology/operational choices must meet the agreed quality gates. A first Goal save should show ordinary save confirmation rather than impact disclosure because the slice has no relationships or impacted artifacts.
