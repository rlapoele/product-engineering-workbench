...
Screen / Views, User Flows and UI Requirements remain independent Product Artifacts. A `design.md` Workbench extension may represent only active artifacts explicitly included in the selected handoff package, after standard target sections and in fixed Screen / View, User Flow, UI Requirement order. It does not infer related artifacts or change package scope. These artifacts are not design-system tokens and the extension does not replace the canonical specification.
