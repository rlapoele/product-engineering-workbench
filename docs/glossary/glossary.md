...
It records a required Name, optional Description and intentionally omitted sections; typed token collections for colors, typography, spacing, rounded shapes and components; and ordered human-readable guidance for Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components, and Do's and Don'ts. Name and Overview are required to save the Definition. Token values are the normative exported values when present; prose provides their rationale and intended application. A token's identifier is chosen by the author: a raw palette or scale such as `blue-600`, a semantic alias such as `action-primary`, and any other useful convention are all valid. Values may be literals or compatible token references. Optional labels or tags help authors organize their work but do not impose a taxonomy. Each component has an author-defined identifier and one or more named configurations; each configuration has its own property values, which are literals or compatible token references, plus optional description and labels or tags.

Every token and component identifier is unique within its collection, and every configuration identifier is unique within its component. References must resolve, be type-compatible with their use, and not create cycles; the Definition does not require semantic naming, a foundation-versus-semantic layering, or any other design-system methodology. A configuration may represent a variant, interaction state, size, theme treatment or combination, but no state or variant name is required or interpreted by the Workbench. Configurations are self-contained and do not inherit properties from their component or one another. A standard section is represented by relevant typed data or prose, or is explicitly listed as intentionally omitted with an optional reason. Resource References remain source material, not token data.

It is authored directly in the UX/UI section. `Add Design System Definition` opens a private draft prefilled with the Project title; Name and Overview are the only fields required to save it. Saved Definitions expose inline actions for their prose, typed collections and intentional omissions. Token entries explicitly choose a literal or compatible reference; a reference picker offers valid existing targets, and creating a missing target preserves and resumes the original draft. Components are created with their first configuration, and each configuration has an author-defined property table; common property names may be suggested but are not required. A source edit is blocked only by source-model errors such as missing required fields, duplicate identifiers, invalid reference resolution, incompatibility or cycles. Renaming a referenced entry preserves and updates its references. Removing an entry or nested property is allowed only when it has no inbound references; otherwise the Workbench shows those references so the author can repoint or remove them first. There is no automatic cascade, replacement or Stale propagation to independent Product Artifacts. Definition changes create ordinary section revisions, affect later handoffs and never alter prior immutable handoff snapshots. Export-profile requirements are assessed later during Prepare Handoff.

Screen / Views, User Flows and UI Requirements remain independent Product Artifacts. A `design.md` Workbench extension may represent only active artifacts explicitly included in the selected handoff package, after standard target sections and in fixed Screen / View, User Flow, UI Requirement order. A Screen / View entry gives its readable reference, Title and Purpose, followed by non-empty Key content and actions, Key states and Visual references blocks. A Visual reference is a labelled link to the managed package-relative file or external URL with an optional Description caption and supplied reference-local Visual Context; it does not embed the file. A User Flow entry gives its readable reference, Title, primary User Profile name, Entry context, Intended outcome, ordered Journey steps and non-empty Intentional exits or Error recovery paths. A step links an included Screen / View entry or retains an absent one as an `Outside package` reference without adding its detail. A UI Requirement entry gives its readable reference, Title, canonical statement, applies-to targets with primary/additional status, and non-empty interaction, accessibility, responsive and resource-reference blocks. Its targets link when included or otherwise remain `Outside package`; resource references are labelled package-relative or external links, not embedded files. Thus a partial package can retain an included Flow or Requirement's necessary excluded target reference without copying its detail or making it implicitly included. The extension does not infer related artifacts or change package scope. These artifacts are not design-system tokens and the extension does not replace the canonical specification.

## Design System Companion Export Profile

A Design System Companion Export Profile is a system-supported, versioned mapping from a Design System Definition to a handoff companion file. It is selected only during Prepare Handoff, together with the choice to include or omit the companion; it is not stored in the Definition. The handoff may offer a visible default, which the author may accept or replace with another supported profile.

The first profile is Google `DESIGN.md` `alpha`, based on the Google Labs Code [DESIGN.md Format pinned for this profile](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md). It maps the Definition's metadata, omissions, token collections and ordered prose to the target format; preserves compatible token references using target `{path.to.token}` syntax; renders component configuration descriptions in the Components guidance; and flattens every component/configuration pair to one `components` entry using the profile's `component--configuration` key. Optional labels and tags remain Workbench organization metadata and are not emitted by default. The profile requires unique flattened keys and valid target references. When Colors are included, it uses a source token named `primary` or an explicit per-handoff primary-color binding to emit the target's required `primary` token. Its recognized component properties are `backgroundColor` and `textColor` (Color), `typography` (a compatible Typography reference), and `rounded`, `padding`, `size`, `height` and `width` (Dimension). A known property that is not target-valid blocks export. Other property names are preserved when representable as target scalar or reference values and produce a warning; only an unrepresentable value or unresolved emitted target reference blocks them.

An Export Profile's mapping is not a live dependency on an external branch. The initial Google profile records target version `alpha`, Workbench mapping revision `1`, canonical source URL and the pinned [upstream specification commit `9bf8eae67128b6cc55ad9bf86665767deb4c11cd`](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md). A later upstream change does not alter that mapping. Any change to mapping behavior, validation or serialization creates a new positive Workbench mapping revision (or a separately supported profile) with its own pinned source revision; it is an explicit future-handoff choice, not an automatic upgrade.
...
When Source Capture produces a saved Product Knowledge record or Revision, its provenance retains a visible link to the one Source used for that capture, including that Source's external-AI indication when the owner selected it. A later capture from another Source may produce another ordinary Revision with its own source provenance. These links provide evidence navigation; they are not Artifact Relationships, source synchronization or a claim that the Source is correct.
...
Every new Source may carry the optional, owner-set **Externally AI-assisted source** indication, which is off by default. It records that external AI materially helped create or summarize that Source; it is a voluntary external-AI declaration in the known AI activity trace. The indication does not establish correctness, verification, final-text authorship or ownership, and MVP does not require provider, model, prompt or cost information. Its visibility beyond the Project Owner follows the existing project AI-use disclosure setting.

---

## Source Note

A Source Note is an owner-authored, non-canonical Source. In the MVP, it has a required concise Title and Evidence text, with the optional Externally AI-assisted source indication. It needs no external URL, provider, model, prompt or cost metadata. Like every Source, it remains evidence until the owner explicitly captures it into reviewed Product Knowledge.
...
Only normal explicit save makes the resulting knowledge canonical. That save retains a visible source-provenance link to the Source; discarding the draft leaves the Source unchanged and creates neither knowledge nor provenance. An externally AI-assisted Source retains its owner-provided indication, but remains evidence to be reinterpreted and reconfirmed by the owner. Source Capture never automatically promotes a Source, an AI interpretation or external content into Product Knowledge, and is not a code-import, synchronization or delivery-management workflow.
...
During Prepare Handoff, an author may choose to include it as a derived design-system companion and select or accept the default Export Profile. That file is an implementation-facing representation, not canonical Product Knowledge or a frontend implementation model. Its optional Workbench extension is a concise projection of only active Screen / Views, User Flows and UI Requirements explicitly included in the prepared package, ordered by type and without relationship-driven scope expansion; those artifacts remain canonical in the Specification.
