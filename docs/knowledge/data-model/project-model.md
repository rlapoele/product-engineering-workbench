...
A **Source** is the non-canonical evidence that informs this understanding: an observation, stakeholder input, document, source-code context, screenshot, prototype, repository, analytics or another external input. Brownfield intake initially accepts three forms: an owner-authored Source Note, a managed uploaded file and an external link. A repository is an external link or supplied file context, not a cloned or synchronized codebase. Sources are added only after the Project exists and initially attach at Specification scope; later Sources may be attached at Specification, Section or Product-Artifact scope through the same model.

**Source Capture** is the Project Owner's explicit act of reviewing one Source and choosing a destination: a new Product Artifact, section-owned Product Knowledge, Decision, Open Question or existing knowledge to revise. It opens the ordinary destination draft with the Source as read-only context. The owner authors and reviews that draft normally; only explicit save makes its record or Revision canonical and retains visible source provenance. Discarding the draft leaves the Source unchanged and creates no knowledge or provenance. A later capture from another Source creates its own ordinary Revision and source provenance rather than a batch reverse-engineering workflow. An external AI may produce a useful summary of an existing codebase outside the Workbench; the owner may add that output as a Source with known origin when supplied, but it remains evidence to reinterpret and reconfirm, not reverse-engineered specification truth. Neither a Source nor an AI interpretation automatically creates Product Knowledge.
...
The MVP should not introduce lower-level design-system Product Artifact types such as Design Token, Color, Typography, UI Component, Wireframe or Prototype. It may instead record a section-owned Design System Definition inside Shared Design Guidance. This has no independent lifecycle, generic relationship model or visual design workspace.

The Definition records required Name, optional Description, intentionally omitted sections, typed color, typography, spacing, rounded-shape and component token collections, and human-readable guidance in the fixed order Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components, and Do's and Don'ts. Name is prefilled from the Project title but remains editable; Name and Overview are the minimum saved Definition. Each token is a structured entry, not a phrase that the system must interpret: colors have CSS color literals or compatible token references; typography records font family, size, weight, line height and optional letter spacing, feature or variation settings; spacing and rounded shapes have dimensions or numbers as applicable. A component has an author-defined identifier and one or more named configurations. Each configuration has its own property values, which are literals or compatible token references, and may have optional description and labels or tags. A token identifier is author-defined: raw scales such as `blue-600`, semantic aliases such as `action-primary`, and other naming conventions are equally valid. Authors may optionally apply labels or tags to organize tokens, but these are descriptive rather than required categories. Token and component identifiers are unique within their collections, configuration identifiers are unique within their component, and references must resolve, be type-compatible with their use, and be acyclic. A configuration may represent a variant, interaction state, size, theme treatment or a combination; no fixed state or variant names are required or interpreted. Configurations are self-contained and do not inherit property values from a component or another configuration. The workbench does not enforce semantic naming, a foundation-versus-semantic split, or any other design-system methodology. A standard section is represented by relevant typed data or prose, or is explicitly listed as intentionally omitted with an optional reason. The current source model and its defined profile conformance are sufficient for the initial scope. Richer presentation controls or component-property vocabulary may be reconsidered only when a concrete handoff need cannot be represented clearly by the current model, a supported profile revision or linked Resource References. Resource References retain external design systems, design files, wireframes, prototypes, screenshots and other source material without making them canonical tokens.

The UX/UI section provides `Add Design System Definition`, which opens a private draft prefilled with the Project title. Required Name and Overview are sufficient to save; Description is optional. The saved Definition renders its standard guidance, typed collections and intentional omissions inline in the section, with ordinary edit and remove actions rather than a separate token workspace. `Add Color`, `Add Typography`, `Add Spacing` and `Add Rounded shape` open collection-specific private drafts. Each value explicitly selects Literal or Token reference. The reference picker presents compatible existing targets; if an author creates a missing target, the original draft is preserved and resumes with it selected. `Add Component` creates a component with its first required named configuration. Existing components expose `Add Configuration`; each configuration uses an author-defined property-name/value table, where values are literals or compatible token references and common property names are suggestions only. A standard section with no relevant data or prose may be marked intentionally omitted with an optional reason; adding data or prose requires clearing that omission. Source save validation checks required fields, uniqueness, resolved compatible references and reference cycles. It does not apply profile-specific naming, primary-color, flattened-key or target-syntax rules; those run only for the chosen Design System Companion Export Profile during Prepare Handoff. Resource References retain external design systems, design files, wireframes, prototypes, screenshots and other source material without making them canonical tokens.

Definition edits use the ordinary section Revision boundary. Editing a value changes future generated companions but preserves references that target it; renaming a referenced token, component, Configuration or reference-target property likewise retains and updates the logical reference. A remove action is available only when its target has no inbound references. Otherwise, the Workbench lists the dependents and requires the author to repoint or remove them before retrying; it never cascades deletion or silently substitutes a value. Removing the whole Definition requires ordinary confirmation and does not delete independent Screen / Views, User Flows, UI Requirements or Resource References. Definition changes do not make those independent Product Artifacts Stale. Previously prepared handoff snapshots remain immutable, while later preparation uses the revised current Definition.

Specific interface behavior remains a UI Requirement; measurable quality remains a Non-Functional Requirement; cross-cutting responsive or accessibility principles may remain Shared Design Guidance. Screen / Views, User Flows and UI Requirements are not Design System Definition tokens.

During Prepare Handoff, an author decides whether to include a derived Design System Companion and selects or accepts the visible default Design System Companion Export Profile. This choice is independent of the Definition, so the same canonical knowledge may be handed off without a companion or through different supported profiles. The first profile, Google `DESIGN.md` `alpha`, Workbench mapping revision `1`, is pinned to [upstream specification commit `9bf8eae67128b6cc55ad9bf86665767deb4c11cd`](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md). It maps metadata, omissions, token collections and ordered prose to its standard format; preserves target-compatible references; renders configuration descriptions in Components guidance; and places any included UX/UI artifact context only after the standard sections in one clearly labelled non-normative Workbench extension. The extension has non-empty Screen / Views, User Flows and UI Requirements subsections in that order, and projects only active artifacts explicitly included in the selected handoff package and in its prepared document order. It does not expand scope through graph relationships or make another selection surface. Each Screen / View projection renders its readable reference and Title, Purpose, then non-empty Key content and actions, Key states and Visual references blocks. A visual reference is a labelled link to its managed package-relative file or external URL, with its optional Description caption and reference-local Visual Context; managed files remain separately copied package resources, not embedded companion content. The projection excludes relationships, Acceptance Criteria, User Profile context and implementation structure. Each User Flow projection renders its readable reference and Title, primary User Profile name, Entry context, Intended outcome, then its ordered Journey steps and any Intentional exits or Error recovery paths. A step retains Action, optional Relevant state and Feedback and continuation. It links its Surface to an included Screen / View extension entry; otherwise it retains the Surface reference and title marked `Outside package`, without adding or linking to absent Screen / View detail. The Flow projection excludes Feature and Functional Requirement relationships, Acceptance Criteria, User Profile detail and implementation structure. Each UI Requirement projection renders its readable reference and Title, canonical Requirement statement, then Applies to, Interaction and state details, Accessibility expectations, Responsive expectations and Resource references blocks when non-empty. Applies-to targets retain their primary or additional status and link to included Screen / View or User Flow entries; otherwise they remain plain references marked `Outside package`. Resource references are labelled managed package-relative or external links with optional Description captions, never embedded files. The UI Requirement projection excludes `depend_on` relationships, Acceptance Criteria, unrelated Feature context and implementation structure. It flattens each Component/configuration pair to a `components` entry named `component--configuration`; a collision blocks that export rather than changing a source name. Labels and tags are authoring metadata and are not exported by default. If Colors are included and no source token is named `primary`, the author must select a per-handoff primary-color binding; the profile emits a derived `primary` alias to that source value, without changing canonical knowledge. The profile adopts the pinned Google Labs Code [DESIGN.md Format](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md) component-property vocabulary: `backgroundColor` and `textColor` are Colors; `typography` is a compatible Typography reference; and `rounded`, `padding`, `size`, `height` and `width` are Dimensions. A known property must use a target-valid literal or compatible reference, emitted with target `{path.to.token}` syntax, or that profile blocks. An unknown property is retained with a warning when it can be emitted unchanged as a target scalar or reference; the profile blocks only an unrepresentable value or unresolved emitted target reference. This does not constrain the free-form source property table. The immutable handoff snapshot retains the inclusion decision, target profile/version, Workbench mapping revision, pinned source reference, bindings, warnings and generated companion. The target profile never auto-updates from its upstream branch: changed mapping behavior, validation or serialization requires a new positive mapping revision or separately supported profile, explicitly selected for a later handoff. The file makes implementation-facing design context portable for human or AI consumers; it is not canonical Product Knowledge or a frontend implementation model, and must not invent unspecified source values.

A Project Workspace validation scenario confirms the complete boundary. An author saves a minimum Definition, then adds literal `brand-blue-600`, semantic `action-primary` referencing it, `body-md` typography, rounded `control`, and a `button` / `primary-hover` configuration. It intentionally omits Elevation & Depth, while the Configuration description is retained and an unknown `borderColor` property remains valid source knowledge. During Prepare Handoff, the author includes the Google profile, binds `action-primary` as the target's required primary color, and receives a companion with a derived `colors.primary`, flattened `components.button--primary-hover`, Components prose, an unknown-property warning and a non-normative extension referencing the relevant Screen / View and UI Requirement. The snapshot retains all of those profile-specific results. A recovery branch uses source identifiers whose different component/configuration pairs flatten to the same target key; source validation remains valid, but the selected profile blocks preparation until one source identifier changes.

A partial-package extension scenario validates the full derived boundary. The package includes `SV-001 Project overview`, `UF-001 Create first project` and `UIR-001 Make project creation clear`, but not `SV-002 Create project dialog`. The Screen / View has a managed empty-overview mockup link with its supplied empty-state and desktop Visual Context. The User Flow links `SV-001` but renders `SV-002 — Create project dialog` only as **Outside package**. The UI Requirement links its primary `SV-001`, retains additional `SV-002` as **Outside package**, renders applicable interaction, accessibility and responsive detail, and links an external form-validation prototype. The generated extension preserves its fixed type order and adds neither an `SV-002` projection nor User Profile, relationship or embedded file content. The immutable snapshot retains the complete generated companion.

The Design System Definition and its initial Google `DESIGN.md` companion are closed for the current scope. Markdown presentation refinements, additional target formats and mapping revisions remain deliberate, evidence-driven future changes; none is implied by upstream movement or added merely to make the model more design-tool-like.
...
Each UI Requirement receives an immutable project-scoped readable reference when created, such as `UIR-001`, in addition to its internal artifact identifier. The reference does not encode its title, Screen / View, User Flow, implementation route, component or accessibility standard. It remains unchanged by editing, applies-to relationship changes and archival. Prepared handoffs retain the included reference and saved UI Requirement snapshot; Acceptance Criteria, UX reviews and derived `design.md` output may use it for unambiguous traceability.
...
The UX/UI section presents UI Requirements in a fixed block with `Add UI Requirement`. Its private draft requires Title, a deliberately selected primary Screen / View or User Flow target and the canonical Requirement statement; `Also applies to` optionally exposes additional explicit targets during the same draft. From a Screen / View or User Flow, creation preselects that target visibly as the changeable primary target. If a needed primary target is absent, Contextual Related-Knowledge Creation preserves the draft while the author explicitly creates the Screen / View or User Flow, then resumes with it selected. Saved UI Requirements have ordinary edit and locally confirmed archive actions, Resource Reference creation and target-preselected `Add Acceptance Criterion` authoring. Promoting an existing additional target to primary retains the former primary as additional because scope is unchanged. Selecting a wholly new primary target requires the author to choose whether to retain the former primary as additional or remove it; no applies-to scope change is implicit.

When a UI Requirement changes or is archived, Acceptance Criteria that directly `validate` it become Stale. Its Screen / View and User Flow applies-to targets receive coverage/readiness cues rather than becoming Stale: their independent interface context or journey may remain accurate while its documented local obligation changes or disappears. A changed or archived applies-to Screen / View or User Flow, or depended-on Functional Requirement, makes the UI Requirement Stale. Primary and additional applies-to targets behave identically for propagation. Changing or archiving a UI Requirement does not automatically make its applies-to targets or depended-on Functional Requirement Stale. A changed Resource Reference is a Revision of its attached UI Requirement and follows this same downstream propagation.
...
| User Flow changes or archival | Acceptance Criteria `validates` User Flow; User Flow `supports` Core Feature | Mark directly validating Acceptance Criteria Stale; create a coverage/readiness cue on supported Core Features. | A changed journey may invalidate its observable evidence while a capability remains valid despite weakened known experience coverage. |
| User Flow validation chain | User Profile; Core Feature; Functional Requirement; Screen / Views; User Flow with journey steps and unhappy paths; Acceptance Criterion validates Flow and behavior | An included View or primary Profile change makes the Flow Stale; a Flow change makes direct evidence Stale and gives the Feature a coverage/readiness cue. | The complete experience journey remains traceable across entry, action, state, feedback, intentional exit and error recovery without turning the model into a route map, test script or technical failure model. |
| UI Requirement changes or archival | Acceptance Criteria `validates` UI Requirement; UI Requirement `applies_to` Screen / View or User Flow | Mark directly validating Acceptance Criteria Stale; create coverage/readiness cues on every applies-to target. | An interface obligation may change or disappear while the surrounding view or journey remains accurate but less completely specified. |
| UI Requirement dependency chain | Screen / View or User Flow; Functional Requirement; UI Requirement `applies_to` target and `depends_on` Requirement | A changed applies-to target or depended-on Requirement makes the UI Requirement Stale; a UI Requirement change does not make those independent artifacts Stale. | The local obligation can depend on its interface and behavior context without making the context depend on one implementation-facing expression of it. |
| UI Requirement integrated validation | Shared Design Guidance; `FEAT-001 Manage projects`; `FR-020 Make project creation available`; `SV-001 Project overview`; `UF-001 Create first project`; Screen/View- and Flow-scoped UI Requirements; direct Acceptance Criteria | Shared guidance supplies common intent; the UI Requirements express separate observable interface obligations, and a criterion may validate each linked target only where its evidence genuinely covers them. UI Requirement, Screen / View, User Flow and Functional Requirement changes produce their respective Stale or coverage outcomes without cross-invalidating independently meaningful knowledge. | The complete UX/UI bundle remains implementation-facing and traceable without turning shared guidance into a design system, a flow into a test script, or local UI obligations into duplicate behavior requirements. |
