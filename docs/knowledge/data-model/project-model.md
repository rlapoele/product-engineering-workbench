...
The Definition records required Name, optional Description, system-managed `design.md` target/version, intentionally omitted sections, typed color, typography, spacing, rounded-shape and component token collections, and human-readable guidance in the fixed order Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components, and Do's and Don'ts. Name is prefilled from the Project title but remains editable; Name and Overview are the minimum saved Definition. Each token is a structured entry, not a phrase that the system must interpret: colors have CSS color values; typography records font family, size, weight, line height and optional letter spacing, feature or variation settings; spacing and rounded shapes have dimensions or numbers as applicable; and component or named-variant property values are literals or token references. Token and component/variant names are unique within their collections. A standard section is represented by relevant typed data or prose, or is explicitly listed as intentionally omitted with an optional reason. The detailed field validation and authoring flow remain deferred. Resource References retain external design systems, design files, wireframes, prototypes, screenshots and other source material without making them canonical tokens.
