...
An Implementation Handoff may render the Definition as a derived `design.md` companion with machine-readable tokens and ordered human-readable guidance. It may add a clearly labelled Workbench extension that references relevant Screen / Views, User Flows, UI Requirements, Resource References and linked quality expectations. The immutable handoff snapshot retains the generated format version. The file makes implementation-facing design context portable for human or AI consumers; it is not canonical Product Knowledge or a frontend implementation model, and must not invent unspecified values.
