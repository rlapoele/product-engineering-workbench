...
The Definition records metadata, intentionally omitted sections, typed color, typography, spacing, rounded-shape and component token collections, and human-readable guidance in the fixed order Overview, Colors, Typography, Layout, Elevation & Depth, Shapes, Components, and Do's and Don'ts. Each token is a structured entry, not a phrase that the system must interpret: colors have CSS color values; typography records its font properties; spacing and rounded shapes have dimensions or numbers as applicable; and component or named-variant property values are literals or token references. The detailed field validation and authoring flow remain deferred. Resource References retain external design systems, design files, wireframes, prototypes, screenshots and other source material without making them canonical tokens.
...
An Implementation Handoff may render the Definition as a derived `design.md` companion with machine-readable tokens and ordered human-readable guidance. It may add a clearly labelled Workbench extension that references relevant Screen / Views, User Flows, UI Requirements, Resource References and linked quality expectations. The file makes implementation-facing design context portable for human or AI consumers; it is not canonical Product Knowledge or a frontend implementation model, and must not invent unspecified values.
