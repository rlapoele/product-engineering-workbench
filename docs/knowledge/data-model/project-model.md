...
A **Source** is the non-canonical evidence that informs this understanding: an observation, stakeholder input, document, source-code context, screenshot, prototype, repository, analytics or another external input. **Source Capture** is the Project Owner's explicit act of reviewing that evidence and beginning an ordinary Product Knowledge draft or revision. A resulting record may retain source provenance, but is canonical only after its normal explicit save. Files and links may use the existing Resource and Resource Reference model; the detailed representation of other source forms is intentionally deferred. Neither a Source nor an AI interpretation automatically creates Product Knowledge.
