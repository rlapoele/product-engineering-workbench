...
An authorized requestor may explicitly dismiss an individual response item as not pursued. The item remains visible as dismissed evidence, with an optional rationale; dismissal does not delete it, reject its contributor, alter Product Knowledge, create a lifecycle-history event or mark the containing response Acted On. From any item, an authorized requestor may open an appropriate ordinary private creation or editing draft—for example, reviewing a Suggested Edit in its target content, or starting a draft for a proposed Artifact, Relationship, Decision or Open Question. The item never creates or applies that draft automatically, and only the ordinary authorized save creates a Revision. The requestor may explicitly mark the entire response Acted On after any mix of consideration, dismissal or separate crystallization; this remains a response-level handling judgment and does not require every item to be dismissed, a linked Revision or a note.
...
An explicit Response Source Link is narrow supporting evidence that a Contribution Response informed a human-authorized Revision. It does not by itself make an exact authorship or derivation claim, and the MVP does not add a separate response-to-Provenance flow.
