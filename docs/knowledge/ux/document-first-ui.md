...
The interface distinguishes **Sources** from canonical Product Knowledge. A Source is non-canonical evidence such as an observation, stakeholder input, document, source-code context, screenshot, prototype, repository or analytics. **Source Capture** is an explicit owner action that uses reviewed source material to begin an ordinary Product Knowledge draft or Revision; the normal authoring and save boundary remains the only route to canonical knowledge. A file or link may use the existing Resource Reference interaction, while the detailed interface for other source forms remains to be decided. No source, external content or AI interpretation silently creates specification content.
