...
After a successful save, the resulting Product Knowledge, Decision or Open Question retains a link to the Conversation and, when selected, the source messages. The Conversation presents a compact derived outcome link without becoming canonical or implying agreement by participants. This action remains available when a Conversation is Closed: it uses the retained thread as evidence, does not reopen it and does not add a Conversation message.
