...
Resource References attached to included scope are included in the handoff. Managed uploaded files are copied into the package with their reference metadata; external links remain labeled links and become caveats when their availability cannot be established. A Resource Reference does not expand the selected handoff scope by itself.
