...
The model does not add a component-inheritance or separate UI-state system, and the MVP must not invent missing source tokens or silently treat prose as a typed value. Richer component-property guidance and presentation refinements remain later decisions.
