...
The Project has a relevant Project overview Screen / View and primary-action UI Requirement. During Prepare Handoff, the author includes the companion, selects or accepts Google `DESIGN.md` `alpha`, and explicitly binds `action-primary` as the profile's required primary color because the source has no `primary` token. The generated companion has a derived `colors.primary`, a flattened `components.button--primary-hover` entry, the Configuration description in Components guidance, an unknown-property warning, and the Screen / View and UI Requirement only in the non-normative Workbench extension. The prepared snapshot retains the inclusion choice, profile/version, binding, warning and generated file.
