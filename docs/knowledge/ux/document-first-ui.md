...
- **Spacing:** token name and a dimension, numeric value or compatible token reference.
- **Rounded shapes:** token name and a dimension value or compatible token reference.
- **Components:** an author-defined component name and one or more named configurations. Each configuration has its own property values, each a literal or compatible token reference, plus optional description and labels or tags.
...
Each token and component name is unique within its collection; configuration names are unique within their parent component. Token identifiers are author-defined: a raw palette or scale such as `blue-600`, a semantic alias such as `action-primary`, and any other useful convention are equally valid. Values may refer to compatible tokens, allowing authors to build semantic aliases from raw values when useful. Optional labels or tags may help authors organize their work but are descriptive rather than required categories. References must resolve, be type-compatible with their use and be acyclic. A component configuration may represent a variant, interaction state, size, theme treatment or a combination—such as `primary-hover`—without requiring or interpreting any particular name. Configurations are self-contained: they do not inherit property values from their component or another configuration, and token references provide reuse. The workbench does not require semantic naming, a foundation-versus-semantic split or another design-system methodology. A standard section is represented by relevant typed data or prose, or explicitly listed as intentionally omitted; an omission may state why. Resource References remain source material, never token data.
...
The UX/UI section exposes **Add Design System Definition** in Shared Design Guidance. It opens a private draft with Name prefilled from the Project title, plus optional Description and required Overview. On save, the Definition appears as an inline section-owned block—never as a separate token workspace—with its standard guidance sections, typed collections and intentional omissions. Authors use ordinary edit and remove actions for saved section-owned entries.

Collection actions expose **Add Color**, **Add Typography**, **Add Spacing** and **Add Rounded shape**. Each opens a collection-specific private draft and deliberately selects a **Literal** value or a **Token reference**. A reference picker offers compatible existing targets. If a required target does not yet exist, the author may create it without losing the original draft; after that target saves, the draft resumes with it selected. Components use **Add Component** to create an author-defined component with its first required named configuration, and **Add Configuration** within a saved component. A configuration has an author-defined property-name/value table; property names remain free-form, with common names offered only as suggestions, and values are literals or compatible token references. A standard section with no relevant data or prose may be explicitly marked intentionally omitted with an optional reason; adding data or prose requires deliberately clearing the omission.

Source validation occurs while saving the relevant Definition or entry. It blocks only missing source-required fields, duplicate identifiers, unresolved or incompatible references and reference cycles. It does not impose a target profile's semantic names, `primary` binding, flattened component keys or value syntax. Those are assessed only when the author chooses a Design System Companion profile during Prepare Handoff.

Editing a token, component, Configuration or property creates an ordinary section Revision. Renaming a referenced entry preserves and updates its logical references. An entry or reference-target property can be removed only when no other entry refers to it. When inbound references exist, the Workbench lists them and requires the author to repoint or remove them; it never cascades removal or substitutes a replacement automatically. Removing the entire Definition is separately confirmed and does not remove independent Screen / Views, User Flows, UI Requirements or Resource References. Definition edits do not make those artifacts Stale; they affect only later handoffs, while prepared snapshots remain immutable.

The Definition is target-independent. During **Prepare Handoff**, the author chooses whether to include a Design System Companion and selects or accepts a visible default **Design System Companion Export Profile**; neither decision is stored in the Definition. The first profile is Google `DESIGN.md` `alpha`, Workbench mapping revision `1`, pinned to [upstream specification commit `9bf8eae67128b6cc55ad9bf86665767deb4c11cd`](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md). It maps the Definition's metadata, omissions and typed token collections to YAML frontmatter; renders its prose in the required standard order; preserves target-compatible token references; and may add Workbench-specific UX/UI context only after the standard sections, under a clearly labelled non-normative extension.

When present, the extension has one `## Workbench Extension (Non-Normative)` heading after all standard `DESIGN.md` sections, then its non-empty subsections in this fixed order: `### Screen / Views`, `### User Flows`, and `### UI Requirements`. It contains only the corresponding active Product Artifacts explicitly included in the selected handoff package, in their prepared document order. The companion does not traverse relationships to infer or add related artifacts, include out-of-package knowledge, or create a second scope selector. Entries remain concise implementation context and retain their ordinary readable references; their complete canonical records remain in the selected Specification content. Included Screen / View Resource References follow the package's ordinary Resource Reference inclusion rules rather than gaining an independent companion-selection path.

Each Screen / View extension entry uses `#### SV-### — Title`, followed by its required Purpose. It then renders non-empty **Key content and actions**, **Key states** and **Visual references** blocks in that order. A Visual reference is a labelled Markdown link to its managed package-relative file or external URL. Its canonical Description may render as a concise caption, and its supplied represented state and/or viewport appear beside that specific reference; no unspecified Visual Context is invented. The companion does not embed or duplicate managed image or other file data: the package carries every managed Resource once, while the reference links to it. The entry does not repeat Feature or Requirement relationships, Acceptance Criteria, User Profile context, implementation routes, component structure or other canonical Specification detail.

Each User Flow extension entry uses `#### UF-### — Title`, followed by its primary User Profile name, Entry context and Intended outcome. Its ordered **Journey steps** render every step's Action, Surface, optional Relevant state and Feedback and continuation. A Surface whose Screen / View has its own extension entry links to that entry. When the Screen / View is not included in the package, the step instead retains plain `SV-### — Title` text marked **Outside package**; it does not add a Screen / View entry, link to unspecified content or expand scope. Non-empty **Intentional exits** and **Error recovery paths** follow Journey steps in that order. The projection excludes Feature and Functional Requirement relationships, Acceptance Criteria, User Profile detail, Screen / View detail, implementation routes and component structure.

Each UI Requirement extension entry uses `#### UIR-### — Title`, followed by its canonical Requirement statement. Its **Applies to** block labels the primary Screen / View or User Flow target and any additional targets. An included target links to its extension entry; an unincluded target remains plain `SV-### — Title` or `UF-### — Title` text marked **Outside package**, without adding target detail or changing scope. Non-empty **Interaction and state details**, **Accessibility expectations**, **Responsive expectations** and **Resource references** blocks follow in that order. Each Resource reference is a labelled link to its managed package-relative file or external URL, with its canonical Description available as a concise caption; files remain package resources rather than embedded content. The projection excludes `depend_on` relationships, Acceptance Criteria, unrelated Feature context, implementation routes and component structure.

For that profile, each Component configuration becomes a flat target entry named `component--configuration`. A duplicate resulting key blocks export rather than renaming source knowledge. Configuration descriptions render in Components guidance; optional labels and tags remain authoring metadata and are not exported by default. Google requires a `primary` color when Colors are included: a source `primary` token fulfils this automatically, otherwise the author selects a per-handoff source-color binding and the export emits a derived `primary` alias. The profile follows the pinned Google Labs Code [DESIGN.md Format](https://github.com/google-labs-code/design.md/blob/9bf8eae67128b6cc55ad9bf86665767deb4c11cd/docs/spec.md) component-property vocabulary: `backgroundColor` and `textColor` require a CSS Color literal or compatible Color reference; `typography` requires a compatible Typography reference; and `rounded`, `padding`, `size`, `height` and `width` require a target-valid Dimension literal (`px`, `em` or `rem`) or compatible Dimension reference. The profile emits references using target `{path.to.token}` syntax. A known property whose literal or reference fails that rule blocks this profile. Any other author-defined property remains source-valid and is emitted unchanged when representable as a target scalar or reference, with an unknown-property warning rather than an invented type rule; an unrepresentable value or unresolved emitted target reference blocks export. The prepared-handoff snapshot retains the companion-inclusion choice, target profile/version, Workbench mapping revision, pinned source reference, bindings, warnings and generated file.

The existing Prepare Handoff scope panel contains a **Design System Companion** card after readiness controls and before the package preview. **Include Design System Companion** is an explicit MVP choice, off by default. Enabling it reveals a Profile selector with a visible default, initially Google `DESIGN.md` `alpha`, and a compact profile preflight: **Ready to include**, non-blocking **Warnings**, or blocking **Needs attention**. The primary-color binding control appears only when the selected profile needs it and lists only compatible source color tokens. A profile change preserves canonical knowledge and reruns that profile's preflight.

The companion is unavailable when the selected package has no included Design System Definition. The card explains why and offers ordinary contextual actions to add it as Supporting Context or create it; it never changes package scope automatically. Turning the option off removes companion-profile validation only; it does not remove ordinary UX/UI guidance already selected for the package. The package preview shows the companion, target profile/version and Workbench mapping revision, plus known warnings and a read-only generated-file preview before preparation. The final snapshot retains the chosen inclusion state, target profile/version, mapping revision, pinned source reference, bindings, warnings and generated file. The Workbench never follows a later upstream-specification change automatically: a behavior change is introduced as a newly supported mapping revision or profile for a later deliberate selection.

### Design System Companion validation

The **Prepare the first-project design companion** scenario validates the complete design-system path. A Project Workspace Definition saves with Name and Overview before optional collections are complete. It then adds `brand-blue-600` as a literal color, `action-primary` as its compatible reference, `body-md` typography, rounded `control`, and a `button` Component with a `primary-hover` Configuration. That Configuration uses `action-primary`, has a description and includes an otherwise unknown `borderColor` property. Elevation & Depth is explicitly omitted because hierarchy instead uses spacing, typography and borders.

The Project has a relevant Project overview Screen / View and primary-action UI Requirement, both explicitly included in the selected package. The Screen / View entry uses `SV-001 — Project overview`, its Purpose, any Key content and actions or Key states, and a labelled link to its included empty-state mockup with a reference-local empty-state and desktop-viewport mention. During Prepare Handoff, the author includes the companion, selects or accepts Google `DESIGN.md` `alpha`, and explicitly binds `action-primary` as the profile's required primary color because the source has no `primary` token. The generated companion has a derived `colors.primary`, a flattened `components.button--primary-hover` entry, the Configuration description in Components guidance, an unknown-property warning, and the Screen / View and UI Requirement only in the non-normative Workbench extension's fixed artifact order. The prepared snapshot retains the inclusion choice, profile/version, binding, warning and generated file.

A recovery branch creates two otherwise valid source Component/configuration pairs whose identifiers flatten to the same Google key—for example, `button` / `primary--hover` and `button--primary` / `hover`. Source validation still succeeds because the names are unique in their respective contexts. Google-profile preparation blocks until the author changes one source identifier, confirming that target-profile collision checks do not constrain canonical naming before an export is selected.

The **Prepare a partial design companion extension** scenario validates the complete extension boundary. The selected package includes the Definition, `SV-001 Project overview`, `UF-001 Create first project` and `UIR-001 Make project creation clear`, but not `SV-002 Create project dialog`. `SV-001` renders its Purpose, empty-state key state and a labelled link to one managed empty-overview mockup, with its supplied empty-state and desktop Visual Context. `UF-001` links its overview step to `SV-001`, while its dialog step retains plain `SV-002 — Create project dialog` text marked **Outside package**. `UIR-001` links its primary `SV-001` target, retains its additional `SV-002` target as **Outside package**, renders its applicable interaction, accessibility and responsive detail, and links its external form-validation prototype. The extension does not add an `SV-002` entry, copy an external source, include User Profile or relationship detail, or embed the managed mockup. Its sections remain Screen / Views, User Flows and UI Requirements in that order, and the prepared snapshot retains the resulting companion.

The model does not add a component-inheritance or separate UI-state system, and the MVP must not invent missing source tokens or silently treat prose as a typed value. The current Definition, Google `DESIGN.md` alpha profile and Workbench-extension projection are sufficient for the initial scope. Richer Markdown presentation controls, additional target formats, mapping revisions and source-model expansion are evidence-driven future decisions: they require a concrete handoff need that cannot be represented clearly by the established model, supported profile evolution or linked Resource References.

### Screen / View direction

A Screen / View is an independent Product Artifact describing a stable user-facing interface context, such as a screen, page, modal, panel or major view. It provides the interface boundary to which future User Flows, UI Requirements and visual Resource References may relate, and supplies screen-level context to implementation handoff. It does not prescribe an implementation route, component tree, wireframe or frontend page architecture.
...
When a Screen / View changes or is archived, User Flows that include it and Acceptance Criteria that directly validate it become Stale. Its supported Core Features receive coverage/readiness cues rather than becoming Stale. A changed or archived Core Feature or Functional Requirement makes an affected Screen / View Stale. Changing or archiving a User Flow does not automatically make its included Screen / Views Stale. A material visual Resource Reference change is a Screen / View revision and follows the same propagation.

### User Flow direction

A User Flow is an independent Product Artifact describing how a person reaches a meaningful outcome across one or more interface contexts. It owns the journey's entry, decisions, feedback, recovery and outcome, and explicitly `includes` Screen / Views. It is distinct from a Use Case, which describes structured actor-and-system interaction for a Core Feature, and from individual Screen / Views, which remain stable reusable interface contexts.

Each User Flow requires a **Title**, primary **User Profile**, **Entry context**, **Intended outcome** and one or more ordered **Journey steps**. A Journey step records required **Action**, selected **Surface** Screen / View, optional **Relevant state**, and required **Feedback and continuation**. Each step supplies the sequence position and any state metadata of its `includes` relationship to the selected view. The flow may also record separate optional **Intentional exits** and **Error recovery paths**; the former captures user-initiated cancellation, back navigation or abandonment, while the latter captures known system-initiated failures or constraints and their user-visible recovery. It never requires authors to invent technical edge cases.

### UI Requirement direction

A UI Requirement is an independent Product Artifact for one specific, observable interface obligation: interaction behavior, visible feedback, state transition, responsive behavior or accessibility expectation. It may apply to a Screen / View or User Flow and trace to related Functional Requirements and Acceptance Criteria. It is distinct from Shared Design Guidance, which provides reusable cross-cutting direction; Screen / View, which provides an interface context; User Flow, which provides a cross-view journey; and Functional Requirement, which defines required system behavior. It must not prescribe CSS, component-library, design-token or frontend implementation choices.

Each UI Requirement requires a **Title**, one deliberately selected **Primary applies-to target**—a Screen / View or User Flow—and a canonical **Requirement statement**. Optional additional applies-to targets avoid duplication where the same obligation genuinely applies elsewhere. Optional **Interaction and state details**, **Accessibility expectations** and **Responsive expectations** are lists that clarify the same canonical obligation only when relevant; they are not completion checklists. Local visual references remain ordinary Resource References. An independently changeable or testable interface obligation becomes a separate UI Requirement even when it shares a target.

Each saved UI Requirement renders its immutable project-scoped readable reference, such as `UIR-001`. This reference is distinct from a title, Screen / View, User Flow, implementation route, component or accessibility standard, and does not change when its details, applies-to relationships or lifecycle state change. Prepared handoffs retain the included reference and saved UI Requirement snapshot; Acceptance Criteria, UX reviews and derived `design.md` output may use it for unambiguous traceability.
...
When a UI Requirement changes or is archived, its directly validating Acceptance Criteria become Stale. Every Screen / View or User Flow it applies to receives a coverage/readiness cue rather than becoming Stale: its independent interface context or journey may remain accurate while its documented local obligation changes or disappears. A changed or archived applies-to Screen / View or User Flow, or depended-on Functional Requirement, makes the UI Requirement Stale. Primary and additional targets behave identically for propagation. Changing or archiving a UI Requirement does not automatically make its applies-to targets or depended-on Functional Requirement Stale. A material Resource Reference change is a UI Requirement revision and follows the same propagation.
...
Journey steps may be added, edited, removed and reordered within the draft; their order determines `includes` relationship sequence metadata. Starting from a Screen / View preselects it as the first step Surface. Starting from a Core Feature or Functional Requirement preselects the optional `supports` or `depends_on` relationship. A saved User Flow exposes `Edit User Flow`, secondary locally confirmed `Archive User Flow`, and `Add Acceptance Criterion` with the Flow preselected as primary validation target.

When a User Flow changes or is archived, its directly validating Acceptance Criteria become Stale and its supported Core Features receive coverage/readiness cues. A changed or archived primary User Profile, included Screen / View, supported Core Feature or depended-on Functional Requirement makes the Flow Stale. Changing or archiving a Flow does not automatically make its included Screen / Views or depended-on Functional Requirements Stale. `relates_to` links to User Stories or Use Cases do not trigger automatic propagation.
...
The interface distinguishes **Sources** from canonical Product Knowledge. A Source is non-canonical evidence such as an observation, stakeholder input, document, source-code context, screenshot, prototype, repository or analytics. After the Project is saved, the Brownfield **Establish context** step offers three initial source actions: **Add Source Note**, **Upload file** and **Add external link**. Every initial Source attaches at Specification scope; the owner may instead choose **Start authoring** at any time. Later Source actions use the same model at Specification, Section or Product-Artifact scope. A repository is linked or supplied as file context, never cloned or synchronized.

**Source Capture** is an explicit owner action that uses reviewed source material to begin an ordinary Product Knowledge draft or Revision; the normal authoring and save boundary remains the only route to canonical knowledge. An owner may obtain an external AI-assisted codebase summary outside the Workbench and add it as a Source, retaining known origin when supplied. It is still evidence to reinterpret and reconfirm rather than a reverse-engineered specification. No source, external content or AI interpretation silently creates specification content.
...
When an included scope contains Shared Design Guidance or relevant UX/UI knowledge, the package may include a derived top-level `design.md`. It represents only available canonical knowledge: shared direction and named values or conventions, plus only explicitly included active Screen / Views and their package-included Resource References, User Flows and UI Requirements. A Screen / View extension entry shows its reference/title, Purpose, non-empty Key content and actions and Key states, then each visual reference as a labelled managed-file or external link with any reference-local Visual Context; managed files remain package files rather than embedded Markdown content. A User Flow entry retains its journey fields and links or marks its Screen / View Surfaces as **Outside package**. A UI Requirement entry has its reference/title, statement, applies-to targets, non-empty interaction, accessibility, responsive and resource-reference blocks; target or resource references follow the same included-link or labelled external/package-file rule. Any such artifact context appears after standard target sections in one clearly labelled non-normative Workbench extension, ordered Screen / Views, User Flows and UI Requirements. The extension never infers related knowledge or expands the package boundary. The companion is portable design context, not the canonical source, a full design system, or a substitute for behavioral requirements and Acceptance Criteria.
