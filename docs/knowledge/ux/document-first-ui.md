...
When the owner creates a new Product Artifact, its selected type determines a canonical destination section. The ordinary creation flow shows that destination but does not offer a placement picker; after the owner saves, the artifact appears at the end of that section. If the section is not selected in the Specification, the local flow asks the owner to add the canonical section or cancel creation. It does not silently change the Specification outline or create an orphaned artifact.
