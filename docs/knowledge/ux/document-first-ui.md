...
The companion is unavailable when the selected package has no included Design System Definition. The card explains why and offers ordinary contextual actions to add it as Supporting Context or create it; it never changes package scope automatically. Turning the option off removes companion-profile validation only; it does not remove ordinary UX/UI guidance already selected for the package. The package preview shows the companion, profile/version and known warnings, with a read-only generated-file preview before preparation. The final snapshot retains the chosen inclusion state, profile/version, bindings, warnings and generated file.
