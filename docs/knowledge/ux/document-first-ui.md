...
When an included scope contains Shared Design Guidance or relevant UX/UI knowledge, the package may include a derived top-level `design.md`. It represents only available canonical knowledge: shared direction and named values or conventions, plus only explicitly included active Screen / Views and their package-included Resource References, User Flows and UI Requirements. Any such artifact context appears after standard target sections in one clearly labelled non-normative Workbench extension, ordered Screen / Views, User Flows and UI Requirements. The extension never infers related knowledge or expands the package boundary. The companion is portable design context, not the canonical source, a full design system, or a substitute for behavioral requirements and Acceptance Criteria.
