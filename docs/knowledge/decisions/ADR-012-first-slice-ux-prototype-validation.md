...
The prototype covers authenticated entry; the calm no-Projects home; Project creation with required title, optional description, editable Content Language pre-filled from the resolved Interface Locale and the fixed starter; empty Specification orientation; local `Add Goal`; inline title/content editing; `Done editing`; saved Goal presentation; and later return to the Project.

It must simulate project-creation and Goal-save pending and failure states. Failures preserve the relevant entered or visible draft data, permit retry and never present false saved state. The saved Goal presents concise confirmation, then receives focus; lifecycle status need not be persistently displayed.
