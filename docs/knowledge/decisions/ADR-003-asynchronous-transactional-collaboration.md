...
The recipient selector groups selectable candidates as **People**—accepted Project Collaborators—and **My assistants**—the requestor's enabled, usable Personal AI Assistants. A configured but unusable assistant may show only a non-selectable `AI assistance unavailable` state. Pending invitees, non-collaborators, the owner as a human recipient and another user's assistant are not selectable. When no eligible and available candidate exists, the flow explains the absent path generically and cannot proceed to the Request Brief or submission; scope and type remain transient local choices, and no Draft, queue, broadcast, fallback assignment, inline invitation or assistant configuration is created. Availability is not live presence, calendar time, willingness, capacity, workload or response-time tracking. A Conversation remains the multi-party path when multiple perspectives or evolving shared context are needed.
