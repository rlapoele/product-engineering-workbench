...
| Journey and accessibility | Playwright covers the essential journey and an intercepted creation/save network failure with safe retry. `@axe-core/playwright` scans the home, creation, editor, saved and error states. The committed automated suite and its source-revision record are the current accessibility evidence. |
...
Automated results are retained as versioned repository evidence through the committed test suite and source-revision evidence records; generated Playwright reports and traces remain transient synthetic-only diagnostics. Manual accessibility evidence is deferred until the MVP is built or its implementation reaches that stage. Before it is run, the repository will add a human-followable VoiceOver/Safari scenario template that records the tested source revision, environment, browser and assistive-technology versions, outcome, defects and known limits. Actual OAuth is exercised in staging with dedicated synthetic test accounts before the first production release and after identity-provider, callback-origin or Better Auth configuration changes; browser tests use test-only sessions rather than a production authentication bypass.
