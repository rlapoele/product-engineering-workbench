...
export const canonicalContentLocale = (value: string): string | null => {
  try {
...
const contentLocale = z.string().trim().min(2, 'Content Language is required.').max(35, 'Content Language is too long.')
  .refine((value) => canonicalContentLocale(value) !== null, 'Use a well-formed language tag.')
  .transform((value) => canonicalContentLocale(value) ?? value);
